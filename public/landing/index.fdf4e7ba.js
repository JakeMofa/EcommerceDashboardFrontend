...
(function(global, factory) {
    typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.SEMICOLON = factory());
})(this, function() {
    // USE STRICT
    "use strict";
    var options = {
    ...
    if (typeof cnvsOptions !== "undefined") options = Object.assign({}, options, cnvsOptions);
    var vars = {
        baseEl: document,
        ...
        get pageTransition () {
            var value1 = this.elBody.classList.contains("page-transition") || options.pageTransition;
            return value1 == "true" || value1 === true ? true : false;
        },
        get tips () {
            var value2 = this.elBody.getAttribute("data-tips") || options.tips;
            return value2 == "true" || value2 === true ? true : false;
        },
        get smoothScroll () {
            var value3 = this.elBody.getAttribute("data-smooth-scroll") || options.smoothScroll;
            return value3 == "true" || value3 === true ? true : false;
        },
        get isRTL () {
    ...
    var Core = function() {
        return {
            ...
            runContainerModules: function(parent1) {
                if (typeof parent1 === "undefined") return false;
                Core.getVars.baseEl = parent1;
                Core.runModules();
            ...
            breakpoints: function() {
                ...
                Object.keys(breakpoint).forEach(function(key) {
                    if (viewWidth > breakpoint[key].enter && viewWidth <= breakpoint[key].exit) vars.elBody.classList.add("device-" + key);
                    else {
                        vars.elBody.classList.remove("device-" + key);
                        if (previous != "") vars.elBody.classList.remove("device-down-" + previous);
                    }
                    if (viewWidth <= breakpoint[key].exit) {
                        if (previous != "") vars.elBody.classList.add("device-down-" + previous);
                    }
                    previous = key;
                    if (viewWidth > breakpoint[key].enter) {
                        vars.elBody.classList.add("device-up-" + key);
                        return;
                    } else vars.elBody.classList.remove("device-up-" + key);
                });
            },
            colorScheme: function() {
                if (vars.elBody.classList.contains("adaptive-color-scheme")) window.matchMedia("(prefers-color-scheme: dark)").matches ? vars.elBody.classList.add("dark") : vars.elBody.classList.remove("dark");
                var bodyColorScheme = Core.cookie.get("__cnvs_body_color_scheme");
                if (bodyColorScheme && bodyColorScheme != "") bodyColorScheme.split(" ").includes("dark") ? vars.elBody.classList.add("dark") : vars.elBody.classList.remove("dark");
            },
            ...
            addEvent: function(el, event, args = {}) {
                if (typeof el === "undefined" || typeof event === "undefined") return;
            ...
            getSelector: function(selector, jquery = true, customjs = true) {
                if (jquery) {
                    if (Core.getVars.baseEl !== document) selector = jQuery(Core.getVars.baseEl).find(selector);
                    else selector = jQuery(selector);
                    if (customjs) {
                        if (typeof customjs == "string") selector = selector.filter(":not(" + customjs + ")");
                        else selector = selector.filter(":not(.customjs)");
                    }
                } else {
                    if (Core.isElement(selector)) selector;
                    else if (customjs) {
                        if (typeof customjs == "string") selector = Core.getVars.baseEl.querySelectorAll(selector + ":not(" + customjs + ")");
                        else selector = Core.getVars.baseEl.querySelectorAll(selector + ":not(.customjs)");
                    } else selector = Core.getVars.baseEl.querySelectorAll(selector);
                }
                return selector;
            },
            ...
            imagesLoaded: function(el) {
                var imgs = el.getElementsByTagName("img") || document.images, len = imgs.length, counter = 0;
                if (len < 1) Core.addEvent(el, "CanvasImagesLoaded");
                var incrementCounter = async function() {
                    counter++;
                    if (counter === len) Core.addEvent(el, "CanvasImagesLoaded");
                };
            ...
            contains: function(classes, selector) {
                ...
                classArray.forEach(function(classTxt) {
                    if (vars.elBody.classList.contains(classTxt)) hasClass = true;
                });
                return hasClass;
            },
            ...
            siblings: function(elem, nodes = false) {
                if (nodes) return [].slice.call(nodes).filter(function(sibling) {
                    return sibling !== elem;
                });
                else return [].slice.call(elem.parentNode.children).filter(function(sibling) {
                    return sibling !== elem;
                });
            },
            ...
            classesFn: function(func, classes, selector) {
                var classArray = classes.split(" ");
                classArray.forEach(function(classTxt) {
                    if (func == "add") selector.classList.add(classTxt);
                    else if (func == "toggle") selector.classList.toggle(classTxt);
                    else selector.classList.remove(classTxt);
                });
            },
            cookie: function() {
                return {
            ...
            scrollTo: function(offset = 0, speed = 1250, easing, behavior = "smooth") {
                if (easing && typeof jQuery !== "undefined" && typeof jQuery.easing["easeOutQuad"] !== "undefined") jQuery("body,html").stop(true).animate({
                    "scrollTop": Number(offset)
                }, Number(speed), easing);
                else {
                    var smoothScroll = "scrollBehavior" in document.documentElement.style;
                    if (typeof window.scroll === "function" && smoothScroll) window.scroll({
                        top: Number(offset),
                        behavior: behavior
                    });
                    else {
            ...
            loadJS: function(params) {
                ...
                if (typeof callback !== "undefined") {
                    if (typeof callback != "function") throw new Error("Not a valid callback!");
                    else htmlScript.onload = callback;
                }
            ...
            topScrollOffset: function() {
                ...
                if (vars.elBody.classList.contains("is-expanded-menu")) {
                    if (vars.elHeader?.classList.contains("sticky-header")) headerHeight = vars.elHeaderWrap.offsetHeight;
    ...
    var Base = function() {
        return {
            ...
            menuBreakpoint: function() {
                if (Core.getVars.menuBreakpoint <= Core.viewport().width) vars.elBody.classList.add("is-expanded-menu");
                else vars.elBody.classList.remove("is-expanded-menu");
                if (vars.elPageMenu) {
                    if (typeof Core.getVars.pageMenuBreakpoint === "undefined") Core.getVars.pageMenuBreakpoint = Core.getVars.menuBreakpoint;
                    if (Core.getVars.pageMenuBreakpoint <= Core.viewport().width) vars.elBody.classList.add("is-expanded-pagemenu");
                    else vars.elBody.classList.remove("is-expanded-pagemenu");
                }
            },
            goToTop: function() {
            ...
            cursor: function() {
                if (vars.customCursor) CNVS.Cursor && CNVS.Cursor.init("body");
            },
            setBSTheme: function() {
                if (vars.elBody.classList.contains("dark")) document.querySelector("html").setAttribute("data-bs-theme", "dark");
                else {
        ...
        return {
            bootstrap: function() {
                ...
                document.querySelectorAll("*").forEach(function(el) {
                    if (notExec) el.getAttributeNames().some(function(text) {
            ...
            resizeVideos: function(element1) {
                CNVS.ResizeVideos && CNVS.ResizeVideos.init(element1 ? element1 : 'iframe[src*="youtube"],iframe[src*="vimeo"],iframe[src*="dailymotion"],iframe[src*="maps.google.com"],iframe[src*="google.com/maps"]');
            },
            pageTransition: function() {
                if (vars.pageTransition) CNVS.PageTransition && CNVS.PageTransition.init("body");
            },
            lazyLoad: function(element1) {
                CNVS.LazyLoad && CNVS.LazyLoad.init(element1 ? element1 : ".lazy:not(.lazy-loaded)");
            },
            ...
            lightbox: function(element1) {
                CNVS.Lightbox && CNVS.Lightbox.init(element1 ? element1 : "[data-lightbox]");
            },
            modal: function(element1) {
                CNVS.Modal && CNVS.Modal.init(element1 ? element1 : ".modal-on-load");
            },
            animations: function(element1) {
                CNVS.Animations && CNVS.Animations.init(element1 ? element1 : "[data-animate]");
            },
            hoverAnimations: function(element1) {
                CNVS.HoverAnimations && CNVS.HoverAnimations.init(element1 ? element1 : "[data-hover-animate]");
            },
            gridInit: function(element1) {
                CNVS.Grid && CNVS.Grid.init(element1 ? element1 : ".grid-container");
            },
            filterInit: function(element1) {
                CNVS.Filter && CNVS.Filter.init(element1 ? element1 : ".grid-filter,.custom-filter");
            },
            canvasSlider: function(element1) {
                CNVS.CanvasSlider && CNVS.CanvasSlider.init(element1 ? element1 : ".swiper_wrapper");
            },
            sliderParallax: function() {
                CNVS.SliderParallax && CNVS.SliderParallax.init(".slider-parallax");
            },
            flexSlider: function(element1) {
                CNVS.FlexSlider && CNVS.FlexSlider.init(element1 ? element1 : ".fslider");
            },
            html5Video: function(element1) {
                CNVS.FullVideo && CNVS.FullVideo.init(element1 ? element1 : ".video-wrap");
            },
            youtubeBgVideo: function(element1) {
                CNVS.YoutubeBG && CNVS.YoutubeBG.init(element1 ? element1 : ".yt-bg-player");
            },
            toggle: function(element1) {
                CNVS.Toggle && CNVS.Toggle.init(element1 ? element1 : ".toggle");
            },
            accordion: function(element1) {
                CNVS.Accordion && CNVS.Accordion.init(element1 ? element1 : ".accordion");
            },
            counter: function(element1) {
                CNVS.Counter && CNVS.Counter.init(element1 ? element1 : ".counter");
            },
            countdown: function(element1) {
                CNVS.Countdown && CNVS.Countdown.init(element1 ? element1 : ".countdown");
            },
            gmap: function(element1) {
                CNVS.GoogleMaps && CNVS.GoogleMaps.init(element1 ? element1 : ".gmap");
            },
            roundedSkills: function(element1) {
                CNVS.RoundedSkills && CNVS.RoundedSkills.init(element1 ? element1 : ".rounded-skill");
            },
            progress: function(element1) {
                CNVS.Progress && CNVS.Progress.init(element1 ? element1 : ".skill-progress");
            },
            twitterFeed: function(element1) {
                CNVS.Twitter && CNVS.Twitter.init(element1 ? element1 : ".twitter-feed");
            },
            flickrFeed: function(element1) {
                CNVS.Flickr && CNVS.Flickr.init(element1 ? element1 : ".flickr-feed");
            },
            instagram: function(element1) {
                CNVS.Instagram && CNVS.Instagram.init(element1 ? element1 : ".instagram-photos");
            },
            // Dribbble Pending
            navTree: function(element1) {
                CNVS.NavTree && CNVS.NavTree.init(element1 ? element1 : ".nav-tree");
            },
            carousel: function(element1) {
                CNVS.Carousel && CNVS.Carousel.init(element1 ? element1 : ".carousel-widget");
            },
            masonryThumbs: function(element1) {
                CNVS.MasonryThumbs && CNVS.MasonryThumbs.init(element1 ? element1 : ".masonry-thumbs");
            },
            notifications: function(element1) {
                CNVS.Notifications && CNVS.Notifications.init(element1 ? element1 : false);
            },
            textRotator: function(element1) {
                CNVS.TextRotator && CNVS.TextRotator.init(element1 ? element1 : ".text-rotater");
            },
            onePage: function(element1) {
                CNVS.OnePage && CNVS.OnePage.init(element1 ? element1 : "[data-scrollto],.one-page-menu");
            },
            ajaxForm: function(element1) {
                CNVS.AjaxForm && CNVS.AjaxForm.init(element1 ? element1 : ".form-widget");
            },
            subscribe: function(element1) {
                CNVS.Subscribe && CNVS.Subscribe.init(element1 ? element1 : ".subscribe-widget");
            },
            conditional: function(element1) {
                CNVS.Conditional && CNVS.Conditional.init(element1 ? element1 : ".form-group[data-condition],.form-group[data-conditions]");
            },
            shapeDivider: function(element1) {
                CNVS.ShapeDivider && CNVS.ShapeDivider.init(element1 ? element1 : ".shape-divider");
            },
            stickySidebar: function(element1) {
                CNVS.StickySidebar && CNVS.StickySidebar.init(element1 ? element1 : ".sticky-sidebar-wrap");
            },
            cookies: function(element1) {
                CNVS.Cookies && CNVS.Cookies.init(element1 ? element1 : ".gdpr-settings,[data-cookies]");
            },
            quantity: function(element1) {
                CNVS.Quantity && CNVS.Quantity.init(element1 ? element1 : ".quantity");
            },
            readmore: function(element1) {
                CNVS.ReadMore && CNVS.ReadMore.init(element1 ? element1 : "[data-readmore]");
            },
            pricingSwitcher: function(element1) {
                CNVS.PricingSwitcher && CNVS.PricingSwitcher.init(element1 ? element1 : ".pricing-tenure-switcher");
            },
            ajaxTrigger: function(element1) {
                CNVS.AjaxTrigger && CNVS.AjaxTrigger.init(element1 ? element1 : "[data-ajax-loader]");
            },
            videoFacade: function(element1) {
                CNVS.VideoFacade && CNVS.VideoFacade.init(element1 ? element1 : ".video-facade");
            },
            schemeToggle: function(element1) {
                CNVS.SchemeToggle && CNVS.SchemeToggle.init(element1 ? element1 : ".body-scheme-toggle");
            },
            clipboardCopy: function(element1) {
                CNVS.Clipboard && CNVS.Clipboard.init(element1 ? element1 : ".clipboard-copy");
            },
            codeHighlight: function(element1) {
                CNVS.CodeHighlight && CNVS.CodeHighlight.init(element1 ? element1 : ".code-highlight");
            },
            tips: function() {
                if (vars.tips) CNVS.Tips && CNVS.Tips.init("body");
            },
            textSplitter: function(element1) {
                CNVS.TextSplitter && CNVS.TextSplitter.init(element1 ? element1 : ".text-splitter");
            },
            mediaActions: function(element1) {
                CNVS.MediaActions && CNVS.MediaActions.init(element1 ? element1 : ".media-wrap");
            },
            viewportDetect: function(element1) {
                CNVS.ViewportDetect && CNVS.ViewportDetect.init(element1 ? element1 : ".viewport-detect");
            },
            scrollDetect: function(element1) {
                CNVS.ScrollDetect && CNVS.ScrollDetect.init(element1 ? element1 : ".scroll-detect");
            },
            fontSizer: function(element1) {
                CNVS.FontSizer && CNVS.FontSizer.init(element1 ? element1 : ".font-sizer");
            },
            hover3D: function(element1) {
                CNVS.Hover3D && CNVS.Hover3D.init(element1 ? element1 : ".hover-3d");
            },
            buttons: function(element1) {
                CNVS.Buttons && CNVS.Buttons.init(element1 ? element1 : ".button-text-effect");
            },
            bsComponents: function(element1) {
                CNVS.BSComponents && CNVS.BSComponents.init(element1 ? element1 : '[data-bs-toggle="tooltip"],[data-bs-toggle="popover"],[data-bs-toggle="tab"],[data-bs-toggle="pill"],.style-msg');
            }
        };
    }();
    var Mobile = function() {
    ...
    var DocumentOnReady = function() {
        return {
            init: function() {
                ...
                if (vars.smoothScroll) new Core.smoothScroll();
...
(function(global, factory) {
    typeof exports === "object" && typeof module !== "undefined" ? module.exports = factory() : typeof define === "function" && define.amd ? define(factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, global.CNVS = factory());
})(this, function() {
    // USE STRICT
    "use strict";
    /**
	 ...
	 * --------------------------------------------------------------------------
	 */ if (SEMICOLON === "undefined" || SEMICOLON.Core === "undefined" || SEMICOLON.Base === "undefined" || SEMICOLON.Modules === "undefined" || SEMICOLON.Mobile === "undefined") return false;
    ...
    return {
		 ...
		 */ Logo: function() {
            return {
                init: function(selector) {
                    ...
                    var head = __core.getVars.elHead, style, css;
		 ...
		 * --------------------------------------------------------------------------
		 */ GoToTop: function() {
            var _init = function(element1) {
                var elSpeed = element1.getAttribute("data-speed") || 700, elEasing = element1.getAttribute("data-easing");
                element1.onclick = function(e) {
                    __core.scrollTo(0, Number(elSpeed), elEasing);
                    e.preventDefault();
                };
            };
            var _scroll = function(element1) {
                var body = __core.getVars.elBody.classList;
                var elMobile = element1.getAttribute("data-mobile") || "false", elOffset = element1.getAttribute("data-offset") || 450;
                if (elMobile == "false" && (body.contains("device-xs") || body.contains("device-sm") || body.contains("device-md"))) return true;
                if (window.scrollY > Number(elOffset)) body.add("gototop-active");
                else body.remove("gototop-active");
            };
            return {
                init: function(selector) {
                    selector = __core.getSelector(selector, false);
		 ...
		 */ StickFooterOnSmall: function() {
            return {
                init: function(selector) {
                    ...
                    var windowH = __core.viewport().height, wrapperH = __core.getVars.elWrapper.offsetHeight;
                    if (!__core.getVars.elBody.classList.contains("sticky-footer") && __core.getVars.elFooter !== "undefined" && __core.getVars.elWrapper.contains(__core.getVars.elFooter)) {
                        if (windowH > wrapperH) __core.getVars.elFooter.style.marginTop = windowH - wrapperH + "px";
                    }
                    if (__core.getVars.elAppMenu) {
                        if (__core.viewport().height - (__core.getVars.elAppMenu.getBoundingClientRect().top + __core.getVars.elAppMenu.getBoundingClientRect().height) === 0) __core.getVars.elFooter.style.marginBottom = __core.getVars.elAppMenu.offsetHeight + "px";
                    }
		 ...
		 */ Headers: function() {
            var _offset = function() {
                ...
                if (__core.getVars.elHeader?.classList.contains("floating-header") || elHeaderInc?.classList.contains("include-topbar")) __core.getVars.headerOffset = __core.offset(elHeader).top;
                ...
                if (elHeader.hasAttribute("data-sticky-offset")) {
                    ...
                    if (headerDefinedOffset == "full") {
                        ...
                        if (typeof headerOffsetNegative !== "undefined") {
                            if (headerOffsetNegative == "auto") __core.getVars.headerWrapOffset = __core.getVars.headerWrapOffset - elHeader.offsetHeight - 1;
                            else __core.getVars.headerWrapOffset = __core.getVars.headerWrapOffset - Number(headerOffsetNegative) - 1;
                        }
                    } else __core.getVars.headerWrapOffset = Number(headerDefinedOffset);
                }
            };
            var _sticky = function(stickyOffset) {
                if (!__core.getVars.elBody.classList.contains("is-expanded-menu") && __core.getVars.mobileSticky != "true") return true;
                if (window.scrollY > stickyOffset) {
                    if (!__core.getVars.elBody.classList.contains("side-header")) {
                        ...
                        if (__core.getVars.elBody.classList.contains("is-expanded-menu") && __core.getVars.stickyShrink == "true" && !__core.getVars.elHeader.classList.contains("no-sticky")) {
                            if (window.scrollY - stickyOffset > Number(__core.getVars.stickyShrinkOffset)) __core.getVars.elHeader.classList.add("sticky-header-shrink");
                            else __core.getVars.elHeader.classList.remove("sticky-header-shrink");
                        }
                    }
                } else {
                    _removeSticky();
                    if (__core.getVars.mobileSticky == "true") _changeMenuClass("responsive");
                }
            };
            var _removeSticky = function() {
                ...
                if (__core.getVars.elHeaderWrap) __core.getVars.elHeaderWrap.className = __core.getVars.headerWrapClasses;
            ...
            var _changeMenuClass = function(type) {
                var newClassesArray = "";
                if ("responsive" == type) {
                    if (__core.getVars.elBody.classList.contains("is-expanded-menu")) return true;
                    if (__core.getVars.mobileHeaderClasses) newClassesArray = __core.getVars.mobileHeaderClasses.split(/ +/);
                } else {
                    if (!__core.getVars.elHeader.classList.contains("sticky-header")) return true;
                ...
                if (noOfNewClasses > 0) {
                    ...
                    for(i = 0; i < noOfNewClasses; i++){
                        if (newClassesArray[i] == "not-dark") {
                            ...
                            if (!__core.getVars.elHeaderWrap?.classList.contains(".not-dark")) __core.getVars.elHeaderWrap?.classList.add("not-dark");
                        } else if (newClassesArray[i] == "dark") {
                            __core.getVars.elHeaderWrap?.classList.remove("not-dark force-not-dark");
                            if (!__core.getVars.elHeader.classList.contains(newClassesArray[i])) __core.getVars.elHeader.classList.add(newClassesArray[i]);
                        } else if (!__core.getVars.elHeader.classList.contains(newClassesArray[i])) __core.getVars.elHeader.classList.add(newClassesArray[i]);
                    }
                }
                __base.setBSTheme();
            };
            ...
            var _sideHeader = function() {
                ...
                if (headerTrigger) headerTrigger.onclick = function(e) {
                    e.preventDefault();
		 ...
		 */ Menus: function() {
            var _init = function() {
                ...
                document.querySelectorAll(".menu-item").forEach(function(el) {
                    if (el.querySelectorAll(".sub-menu-container").length > 0) el.classList.add("sub-menu");
            ...
            var _reset = function() {
                var body = __core.getVars.elBody, subMenusSel = ".mega-menu-content, .sub-menu-container", menuItemSel = ".menu-item";
                document.querySelectorAll(".primary-menu-trigger").forEach(function(el) {
                    el.classList.remove("primary-menu-trigger-active");
                });
                __core.getVars.elPrimaryMenus.forEach(function(el) {
                    if (!body.classList.contains("is-expanded-menu")) el.querySelector(".menu-container")?.classList.remove("d-block");
                    else {
            ...
            var _invert = function(subMenuEl) {
                ...
                // 	return false;
                // }
                if (subMenus.length < 1) return false;
                var primaryMenus;
                subMenus.forEach(function(el) {
                    ...
                    if (el.closest(".mega-menu-small")) {
                        ...
                        if (outside < 0) el.style.left = outside + "px";
                    }
                    if (__core.getVars.elBody.classList.contains("rtl")) {
                        if (viewportOffset.left < 0) el.classList.add("menu-pos-invert");
                    }
                    if (__core.viewport().width - (viewportOffset.left + viewportOffset.width) < 0) el.classList.add("menu-pos-invert");
                });
                subMenus.forEach(function(el) {
                    var elChildren = el.querySelectorAll(":scope > *");
                    elChildren.forEach(function(elChild) {
            ...
            var _hover = function() {
                ...
                if (!isNaN(menuHoverDelay.split("ms")[0])) menuHoverDelay = menuHoverDelay.split("ms")[0];
                else if (!isNaN(menuHoverDelay.split("s")[0])) menuHoverDelay = menuHoverDelay.split("s")[0] * 1000;
            ...
            var _functions = function() {
                var subMenusSel = ".mega-menu-content, .sub-menu-container", menuItemSel = ".menu-item", subMenuSel = ".sub-menu", subMenuTriggerSel = ".sub-menu-trigger", body = __core.getVars.elBody.classList;
				 ...
				 */ if (!body.contains("is-expanded-menu")) {
                    ...
                    triggers.forEach(function(trigger) {
                        trigger.onclick = function(e) {
                            ...
                            if (!trigger.classList.contains("sub-menu-trigger")) triggerEl = trigger.closest(menuItemSel).querySelector(":scope > " + subMenuTriggerSel);
				 ...
				 */ if (body.contains("is-expanded-menu")) {
                    if (body.contains("side-header") || body.contains("overlay-menu")) __core.getVars.elPrimaryMenus.forEach(function(pMenu) {
				 ...
				 */ document.querySelectorAll(".top-links").forEach(function(item) {
                    if (item.classList.contains("on-click") || !body.contains("device-up-lg")) item.querySelectorAll(".top-links-item").forEach(function(menuItem) {
            ...
            var _triggerState = function(triggerEl, menuItemSel, subMenusSel, subMenuTriggerSel, classCheck) {
                ...
                if (classCheck == "d-none") {
                    if (triggerredSubMenus.classList.contains("d-none")) triggerredSubMenus.classList.remove("d-none");
                    else childSubMenus.forEach(function(item) {
                        item.classList.add("d-none");
                    });
                } else if (triggerredSubMenus.classList.contains("d-block")) childSubMenus.forEach(function(item) {
                    item.classList.remove("d-block");
                });
                else triggerredSubMenus.classList.add("d-block");
            ...
            var _trigger = function() {
                ...
                document.querySelectorAll(".primary-menu-trigger").forEach(function(menuTrigger) {
                    menuTrigger.onclick = function(e) {
                        ...
                        if (__core.filtered(__core.getVars.elPrimaryMenus, elTarget).length < 1) return;
                        if (!body.contains("is-expanded-menu")) __core.getVars.elPrimaryMenus.forEach(function(el) {
                            if (el.querySelectorAll(".mobile-primary-menu").length > 0) el.matches(elTarget) && el.querySelectorAll(".mobile-primary-menu").forEach(function(elem) {
                                elem.classList.toggle("d-block");
                            });
                            else el.matches(elTarget) && el.querySelectorAll(".menu-container").forEach(function(elem) {
                                elem.classList.toggle("d-block");
                            });
                        });
                        ...
                        if (elTarget != "*") body.toggle("primary-menu-open-" + elTarget.replace(/[^a-zA-Z0-9-]/g, ""));
                        else body.toggle("primary-menu-open-all");
                    };
                });
            };
            var _fullWidth = function() {
                ...
                if (__core.getVars.elHeader.querySelectorAll(".container-fullwidth").length > 0) document.querySelectorAll(".mega-menu:not(.mega-menu-full):not(.mega-menu-small) .mega-menu-content").forEach(function(el) {
                    el.style.width = headerWidth + "px";
                });
                document.querySelectorAll(".mega-menu:not(.mega-menu-full):not(.mega-menu-small) .mega-menu-content, .top-search-form").forEach(function(el) {
                    el.style.width = headerWidth + "px";
                });
                if (__core.getVars.elHeader.classList.contains("full-header")) document.querySelectorAll(".mega-menu:not(.mega-menu-full):not(.mega-menu-small) .mega-menu-content").forEach(function(el) {
                    el.style.width = headerWidth + "px";
                });
            ...
            return {
                init: function(selector) {
                    ...
                    __core.getVars.resizers.menus = function() {
                        if (windowWidth != __core.viewport().width) __base.menus();
                    };
		 ...
		 */ PageMenu: function() {
            var _sticky = function(stickyOffset) {
                ...
                if (window.scrollY > stickyOffset) {
                    if (__core.getVars.elBody.classList.contains("device-up-lg")) pageMenu.classList.add("sticky-page-menu");
                    else if (pageMenu.getAttribute("data-mobile-sticky") == "true") pageMenu.classList.add("sticky-page-menu");
                } else pageMenu.classList.remove("sticky-page-menu");
            };
            return {
                init: function(selector) {
                    selector = __core.getSelector(selector, false);
                    if (selector.length < 1) return true;
                    var pageMenu = __core.getVars.elPageMenu, pageMenuWrap = pageMenu.querySelector("#page-menu-wrap"), pageMenuClone = pageMenu.querySelector(".page-menu-wrap-clone");
                    if (!pageMenuClone) {
                    ...
                    document.addEventListener("click", function(e) {
                        if (!e.target.closest("#page-menu")) pageMenu.classList.remove("page-menu-open");
                    }, false);
                    if (pageMenu.classList.contains("no-sticky") || pageMenu.classList.contains("dots-menu")) return true;
                    var headerHeight;
                    if (__core.getVars.elHeader.classList.contains("no-sticky")) headerHeight = 0;
                    else if (__core.getVars.elHeader.getAttribute("data-sticky-shrink") == "false") headerHeight = getComputedStyle(__core.getVars.elHeader).getPropertyValue("--cnvs-header-height").split("px")[0];
                    else headerHeight = getComputedStyle(__core.getVars.elHeader).getPropertyValue("--cnvs-header-height-shrink").split("px")[0];
            ...
            return {
                init: function(selector) {
                    ...
                    if (body.classList.contains("device-up-lg")) {
                        setTimeout(function() {
                            if (slInner) slInner.style.height = parallaxElHeight + "px";
                            if (slFlexHeight) {
                        ...
                        if (slFlexHeight && slSwiperS) {
                            ...
                            if (slSwiperFC.classList.contains("container") || slSwiperFC.classList.contains("container-fluid")) slSwiperFC = slSwiperFC.querySelector("*");
                        ...
                        if (!body.classList.contains("stretched")) {
                            ...
                            if (slInner) slInner.style.width = parallaxElWidth + "px";
                        }
                    } else {
                        if (slSwiperW) slSwiperW.style.height = "";
		 ...
		 */ SliderMenuClass: function() {
            ...
            var _schemeChanger = function(activeSlide) {
                ...
                var darkExists = false, oldClassesArray, noOfOldClasses;
                if (activeSlide.classList.contains("dark")) {
                    if (__core.getVars.headerClasses) oldClassesArray = __core.getVars.headerClasses;
                    else oldClassesArray = "";
                    noOfOldClasses = oldClassesArray.length;
                    if (noOfOldClasses > 0) {
                        for(var i = 0; i < noOfOldClasses; i++)if (oldClassesArray[i] == "dark") {
                            darkExists = true;
                            break;
                        }
                    }
                    var headerToChange = document.querySelector("#header.transparent-header:not(.sticky-header,.semi-transparent,.floating-header)");
                    if (headerToChange) headerToChange.classList.add("dark");
                    if (!darkExists) {
                        ...
                        if (headerToChange) headerToChange.classList.remove("dark");
                    }
                    __core.getVars.elHeaderWrap.classList.remove("not-dark");
                } else if (__core.getVars.elBody.classList.contains("dark")) {
                ...
                if (__core.getVars.elHeader.classList.contains("sticky-header")) __base.headers();
            };
            return {
                init: function(selector) {
                    selector = __core.getSelector(selector, false);
		 ...
		 */ TopSearch: function() {
            return {
                init: function(selector) {
                    ...
                    var topSearchParent = document.querySelector(".top-search-parent"), timeout;
                    selector[0].onclick = function(e) {
                        ...
                        if (__core.getVars.elBody.classList.contains("top-search-open")) topSearchParent?.classList.add("position-relative");
                        ...
                        if (__core.getVars.elBody.classList.contains("top-search-open")) searchForm.querySelector("input").focus();
                    };
                    document.addEventListener("click", function(e) {
		 ...
		 */ TopCart: function() {
            return {
                init: function(selector) {
                    selector = __core.getSelector(selector, false);
                    if (selector.length < 1) return true;
                    if (!document.getElementById("top-cart-trigger")) return false;
                    ...
                    document.addEventListener("click", function(e) {
                        if (!e.target.closest("#top-cart")) selector[0].classList.remove("top-cart-open");
                    }, false);
                }
            };
        }(),
		 ...
		 */ SidePanel: function() {
            return {
                init: function(selector) {
                    ...
                    var body = __core.getVars.elBody.classList;
                    document.addEventListener("click", function(e) {
                        if (!e.target.closest("#side-panel") && !e.target.closest(".side-panel-trigger")) body.remove("side-panel-open");
                    }, false);
                    document.querySelectorAll(".side-panel-trigger").forEach(function(el) {
                        el.onclick = function(e) {
                            e.preventDefault();
                            body.toggle("side-panel-open");
                            if (body.contains("device-touch") && body.contains("side-push-panel")) body.toggle("ohidden");
                        };
                    });
                }
            };
            ...
            return {
                init: function(selector) {
                    ...
                    var adaptClasses = function(dark) {
                        if (dark) __core.getVars.elBody.classList.add("dark");
                        else __core.getVars.elBody.classList.remove("dark");
                        if (__core.getVars.elBody.contains(adaptiveEl)) {
                            if (dark) {
            ...
            var _load = function(portPostId, prevPostPortId, getIt) {
                ...
                if (getIt == false) {
                    ...
                    fetch(portfolioDataLoader).then(function(response) {
                        ...
                        var nextPortfolio = document.getElementById("next-portfolio"), prevPortfolio = document.getElementById("prev-portfolio");
            ...
            return {
                init: function(selector) {
                    ...
                    selector[0].querySelectorAll(".portfolio-ajax-trigger").forEach(function(el) {
                        if (!el.querySelector("i:nth-child(2)")) el.innerHTML += '<i class="bi-arrow-repeat icon-spin"></i>';
                        el.onclick = function(e) {
                            e.preventDefault();
                            var portPostId = e.target.closest(".portfolio-item").getAttribute("id");
                            if (!e.target.closest(".portfolio-item").classList.contains("portfolio-active")) _load(portPostId, __core.getVars.portfolioAjax.prevItem);
                        };
                    });
                }
            };
		 ...
		 */ Cursor: function() {
            return {
                init: function(selector) {
                    ...
                    var addCursorEl = function(selector, parent1) {
                        var el = document.createElement("div");
                        el.classList.add(selector.split(".")[1]);
                        parent1.prepend(el);
                        return document.querySelector(selector);
                    };
                    if (!cursor) cursor = addCursorEl(".cnvs-cursor", __core.getVars.elWrapper);
		 ...
		 */ PageTransition: function() {
            return {
                init: function(selector) {
                    ...
                    var elAnimIn = body.getAttribute("data-animation-in") || "fadeIn", elSpeedIn = body.getAttribute("data-speed-in") || 1000, elTimeoutActive = false, elTimeout = body.getAttribute("data-loader-timeout"), elLoader = body.getAttribute("data-loader"), elLoaderColor = body.getAttribute("data-loader-color"), elLoaderHtml = body.getAttribute("data-loader-html"), elLoaderAppend = "", elLoaderCSSVar = "";
                    if (!elTimeout) {
                    ...
                    if (elLoaderColor) {
                        if (elLoaderColor == "theme") elLoaderCSSVar = ' style="--cnvs-loader-color:var(--cnvs-themecolor);"';
                        else elLoaderCSSVar = ' style="--cnvs-loader-color:' + elLoaderColor + ';"';
                    }
                    var elLoaderBefore = '<div class="css3-spinner"' + elLoaderCSSVar + ">", elLoaderAfter = "</div>";
                    if (elLoader == "2") elLoaderAppend = '<div class="css3-spinner-flipper"></div>';
                    else if (elLoader == "3") elLoaderAppend = '<div class="css3-spinner-double-bounce1"></div><div class="css3-spinner-double-bounce2"></div>';
                    else if (elLoader == "4") elLoaderAppend = '<div class="css3-spinner-rect1"></div><div class="css3-spinner-rect2"></div><div class="css3-spinner-rect3"></div><div class="css3-spinner-rect4"></div><div class="css3-spinner-rect5"></div>';
                    else if (elLoader == "5") elLoaderAppend = '<div class="css3-spinner-cube1"></div><div class="css3-spinner-cube2"></div>';
                    else if (elLoader == "6") elLoaderAppend = '<div class="css3-spinner-scaler"></div>';
                    else if (elLoader == "7") elLoaderAppend = '<div class="css3-spinner-grid-pulse"><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></div>';
                    else if (elLoader == "8") elLoaderAppend = '<div class="css3-spinner-clip-rotate"><div></div></div>';
                    else if (elLoader == "9") elLoaderAppend = '<div class="css3-spinner-ball-rotate"><div></div><div></div><div></div></div>';
                    else if (elLoader == "10") elLoaderAppend = '<div class="css3-spinner-zig-zag"><div></div><div></div></div>';
                    else if (elLoader == "11") elLoaderAppend = '<div class="css3-spinner-triangle-path"><div></div><div></div><div></div></div>';
                    else if (elLoader == "12") elLoaderAppend = '<div class="css3-spinner-ball-scale-multiple"><div></div><div></div><div></div></div>';
                    else if (elLoader == "13") elLoaderAppend = '<div class="css3-spinner-ball-pulse-sync"><div></div><div></div><div></div></div>';
                    else if (elLoader == "14") elLoaderAppend = '<div class="css3-spinner-scale-ripple"><div></div><div></div><div></div></div>';
                    else elLoaderAppend = '<div class="css3-spinner-bounce1"></div><div class="css3-spinner-bounce2"></div><div class="css3-spinner-bounce3"></div>';
                    if (!elLoaderHtml) elLoaderHtml = elLoaderAppend;
                    elLoaderHtml = elLoaderBefore + elLoaderHtml + elLoaderAfter;
                    if (elAnimIn == "fadeIn") __core.getVars.elWrapper.classList.add("op-1");
                    else __core.getVars.elWrapper.classList.add("not-animated");
                    if (!pageTransition) {
                    ...
                    if (elSpeedIn) {
                        ...
                        if (elAnimIn == "fadeIn") pageTransition.style.setProperty("--cnvs-animate-duration", Number(elSpeedIn) + "ms");
                    }
                    ...
                    if (document.readyState === "complete") endPageTransition();
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        window.lazyLoadInstance = new LazyLoad({
                            ...
                            callback_loaded: function(el) {
                                ...
                                if (el.parentNode.getAttribute("data-lazy-container") == "true") __core.runContainerModules(el.parentNode);
                            }
                        });
                    });
                }
            ...
            return {
                init: function(selector) {
                    ...
                    selector.forEach(function(el) {
                        ...
                        if (classes.length > 0) classes.forEach(function(_class) {
                            var deviceClass = _class.split(":");
                            if (__core.getVars.elBody.classList.contains(deviceClass[0] == "dark" ? deviceClass[0] : "device-" + deviceClass[0])) el.classList.add(deviceClass[1]);
                            else el.classList.remove(deviceClass[1]);
                        });
                    });
		 ...
		 */ DataHeights: function() {
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(el) {
                        var heightXs = el.getAttribute("data-height-xs") || "auto", heightSm = el.getAttribute("data-height-sm") || heightXs, heightMd = el.getAttribute("data-height-md") || heightSm, heightLg = el.getAttribute("data-height-lg") || heightMd, heightXl = el.getAttribute("data-height-xl") || heightLg, heightXxl = el.getAttribute("data-height-xxl") || heightXl, body = __core.getVars.elBody.classList, elHeight;
                        if (body.contains("device-xs")) elHeight = heightXs;
                        else if (body.contains("device-sm")) elHeight = heightSm;
                        else if (body.contains("device-md")) elHeight = heightMd;
                        else if (body.contains("device-lg")) elHeight = heightLg;
                        else if (body.contains("device-xl")) elHeight = heightXl;
                        else if (body.contains("device-xxl")) elHeight = heightXxl;
                        if (elHeight) el.style.height = !isNaN(elHeight) ? elHeight + "px" : elHeight;
                    });
                ...
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        selector.each(function() {
                            var element1 = jQuery(this), elType = element1.attr("data-lightbox"), elCloseButton = element1.attr("data-close-button") || "outside", elDisableUnder = element1.attr("data-disable-under") || 600, elFixedContent = element1.attr("data-content-position") || "auto", elZoom = element1.attr("data-zoom"), $body = jQuery("body");
                            ...
                            if (elType == "image") {
                                ...
                                element1.magnificPopup(settings);
                            }
                            if (elType == "gallery") {
                                if (element1.find('a[data-lightbox="gallery-item"]').parent(".clone").hasClass("clone")) element1.find('a[data-lightbox="gallery-item"]').parent(".clone").find('a[data-lightbox="gallery-item"]').attr("data-lightbox", "");
                                if (element1.find('a[data-lightbox="gallery-item"]').parents(".cloned").hasClass("cloned")) element1.find('a[data-lightbox="gallery-item"]').parents(".cloned").find('a[data-lightbox="gallery-item"]').attr("data-lightbox", "");
                                element1.magnificPopup({
                                    delegate: element1.hasClass("grid-container-filterable") ? 'a.grid-lightbox-filtered[data-lightbox="gallery-item"]' : 'a[data-lightbox="gallery-item"]',
                                    type: "image",
                                    tLoading: "",
                                    closeOnContentClick: true,
                                    closeBtnInside: elCloseButton,
                            ...
                            if (elType == "iframe") element1.magnificPopup({
                            ...
                            if (elType == "inline") element1.magnificPopup({
                                type: "inline",
                                tLoading: "",
                                mainClass: "mfp-no-margins mfp-fade",
                                closeBtnInside: elCloseButton,
                            ...
                            if (elType == "ajax") element1.magnificPopup({
                                type: "ajax",
                                tLoading: "",
                                closeBtnInside: elCloseButton,
                                autoFocusLast: false,
                            ...
                            if (elType == "ajax-gallery") element1.magnificPopup({
                            ...
                            element1.on("mfpOpen", function() {
                                var lightboxItem = jQuery.magnificPopup.instance.currItem.el, lightboxClass = jQuery(lightboxItem).attr("data-lightbox-class"), lightboxBgClass = jQuery(lightboxItem).attr("data-lightbox-bg-class");
                                if (lightboxClass != "") jQuery(jQuery.magnificPopup.instance.container).addClass(lightboxClass);
                                if (lightboxBgClass != "") jQuery(jQuery.magnificPopup.instance.bgOverlay).addClass(lightboxBgClass);
                            });
                        });
                    });
                }
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        var closeButtonIcon = '<i class="bi-x-lg"></i>';
                        selector.each(function() {
                            var element1 = jQuery(this), elTarget = element1.attr("data-target"), elTargetValue = "__cnvs_" + elTarget.split("#")[1], elDelay = element1.attr("data-delay") || 500, elTimeout = element1.attr("data-timeout"), elAnimateIn = element1.attr("data-animate-in"), elAnimateOut = element1.attr("data-animate-out"), elBgClick = element1.attr("data-bg-click"), elCloseBtn = element1.attr("data-close-btn"), elCookies = element1.attr("data-cookies"), elCookiePath = element1.attr("data-cookie-path"), elCookieExp = element1.attr("data-cookie-expire");
                            if (elCookies == "false") __core.cookie.remove(elTargetValue);
                            if (elCookies == "true") {
                                var elementCookie = __core.cookie.get(elTargetValue);
                                if (typeof elementCookie !== "undefined" && elementCookie == "0") return true;
                            }
                            if (elBgClick == "false") elBgClick = false;
                            else elBgClick = true;
                            if (elCloseBtn == "false") elCloseBtn = false;
                            else elCloseBtn = true;
                            ...
                            setTimeout(function() {
                                jQuery.magnificPopup.open({
                                    ...
                                    callbacks: {
                                        open: function() {
                                            if (elAnimateIn != "") jQuery(elTarget).addClass(elAnimateIn + " animated");
                                        },
                                        beforeClose: function() {
                                            if (elAnimateOut != "") jQuery(elTarget).removeClass(elAnimateIn).addClass(elAnimateOut);
                                        },
                                        afterClose: function() {
                                            if (elAnimateIn != "" || elAnimateOut != "") jQuery(elTarget).removeClass(elAnimateIn + " " + elAnimateOut + " animated");
                                        }
                                    }
                                }, 0);
                            }, elDelay);
                            if (document.querySelector(".modal-cookies-close")) document.querySelector(".modal-cookies-close").onclick = function() {
                            ...
                            if (elTimeout != "") setTimeout(function() {
                                jQuery.magnificPopup.close();
		 ...
		 */ Animations: function() {
            return {
                init: function(selector) {
                    ...
                    selector = __core.getSelector(selector, false);
                    if (selector.length < 1) return true;
                    var SELECTOR = "[data-animate]", ANIMATE_CLASS_NAME = "animated";
                    var isAnimated = function(element1) {
                        element1.classList.contains(ANIMATE_CLASS_NAME);
                    };
                    var intersectionObserver = new IntersectionObserver(function(entries, observer) {
                        entries.forEach(function(entry) {
                            var element1 = entry.target, elAnimation = element1.getAttribute("data-animate"), elAnimOut = element1.getAttribute("data-animate-out"), elAnimDelay = element1.getAttribute("data-delay"), elAnimDelayOut = element1.getAttribute("data-delay-out"), elAnimDelayTime = 0, elAnimDelayOutTime = 3000, elAnimations = elAnimation.split(" ");
                            if (element1.closest(".fslider.no-thumbs-animate")) return true;
                            if (element1.closest(".swiper-slide")) return true;
                            if (elAnimDelay) elAnimDelayTime = Number(elAnimDelay) + 500;
                            else elAnimDelayTime = 500;
                            if (elAnimOut && elAnimDelayOut) elAnimDelayOutTime = Number(elAnimDelayOut) + elAnimDelayTime;
                            if (!element1.classList.contains("animated")) {
                                element1.classList.add("not-animated");
                                if (entry.intersectionRatio > 0) {
                                    setTimeout(function() {
                                        element1.classList.remove("not-animated");
                                        elAnimations.forEach(function(item) {
                                            element1.classList.add(item);
                                        });
                                        element1.classList.add("animated");
                                    }, elAnimDelayTime);
                                    if (elAnimOut) setTimeout(function() {
                                        elAnimations.forEach(function(item) {
                            ...
                            if (!element1.classList.contains("not-animated")) observer.unobserve(element1);
                        });
                    });
                    var elements = [].filter.call(document.querySelectorAll(SELECTOR), function(element1) {
                        return !isAnimated(element1, ANIMATE_CLASS_NAME);
                    });
                    elements.forEach(function(element1) {
                        return intersectionObserver.observe(element1);
                    });
                }
            };
        }(),
		 ...
		 */ HoverAnimations: function() {
            ...
            var _hideOverlay = function(params) {
                ...
                if (params.elReset == "true") _x = setTimeout(function() {
                    (params.elAnimateOut + " animated").split(" ").forEach(function(_class) {
                        params.element.classList.remove(_class);
                    });
                    params.element.classList.add("not-animated");
            ...
            return {
                init: function(selector) {
                    ...
                    selector = __core.getSelector(selector, false);
                    if (selector.length < 1) return true;
                    selector.forEach(function(element1) {
                        var elAnimate = element1.getAttribute("data-hover-animate"), elAnimateOut = element1.getAttribute("data-hover-animate-out") || "fadeOut", elSpeed = element1.getAttribute("data-hover-speed") || 600, elDelay = element1.getAttribute("data-hover-delay"), elParent = element1.getAttribute("data-hover-parent"), elReset = element1.getAttribute("data-hover-reset") || "false", elMobile = element1.getAttribute("data-hover-mobile") || "true";
                        if (elMobile != "true") {
                            if (elMobile == "false") {
                                if (!__core.getVars.elBody.classList.contains("device-up-lg")) return true;
                            } else {
                                if (!__core.getVars.elBody.classList.contains("device-up-" + elMobile)) return true;
                            }
                        }
                        element1.classList.add("not-animated");
                        if (!elParent) {
                            if (element1.closest(".bg-overlay")) elParent = element1.closest(".bg-overlay");
                            else elParent = element1;
                        } else if (elParent == "self") elParent = element1;
                        else elParent = element1.closest(elParent);
                        var elDelayT = 0;
                        if (elDelay) elDelayT = Number(elDelay);
                        if (elSpeed) element1.style.animationDuration = Number(elSpeed) + "ms";
                        var params = {
                            element: element1,
                            elAnimate: elAnimate,
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elTransition = element1.attr("data-transition") || "0.65s", elLayoutMode = element1.attr("data-layout") || "masonry", elStagger = element1.attr("data-stagger") || 0, elBase = element1.attr("data-basewidth") || ".portfolio-item:not(.wide):eq(0)", elOriginLeft = true, elGrid;
                            _reLayout(element1);
                            if (__core.getVars.isRTL) elOriginLeft = false;
                            if (element1.hasClass("portfolio") || element1.hasClass("post-timeline")) elGrid = element1.filter(":not(.has-init-isotope)").isotope({
                                ...
                                masonry: {
                                    columnWidth: element1.find(elBase)[0]
                                }
                            });
                            else elGrid = element1.filter(":not(.has-init-isotope)").isotope({
                                layoutMode: elLayoutMode,
                                isOriginLeft: elOriginLeft,
                                transitionDuration: elTransition,
                                stagger: Number(elStagger),
                            ...
                            if (element1.data("isotope")) element1.addClass("has-init-isotope");
                            var int = setInterval(function() {
                                if (element1.find(".lazy.lazy-loaded").length == element1.find(".lazy").length) {
                                    setTimeout(function() {
                                        _reLayout(element1);
                                    }, 666);
                                    clearInterval(int);
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elCon = element1.attr("data-container"), elActClass = element1.attr("data-active-class"), elDefFilter = element1.attr("data-default");
                            if (!elActClass) elActClass = "activeFilter";
                            if (!jQuery(elCon).hasClass("grid-container")) return false;
                            element1.find("a").off("click").on("click", function() {
                                element1.find("li").removeClass(elActClass);
                            ...
                            if (elDefFilter) {
                                element1.find("li").removeClass(elActClass);
                                element1.find('[data-filter="' + elDefFilter + '"]').parent("li").addClass(elActClass);
                                jQuery(elCon).isotope({
                                    filter: elDefFilter
                                });
                            }
                        ...
                        jQuery(".grid-shuffle").off("click").on("click", function() {
                            var element1 = jQuery(this), elCon = element1.attr("data-container");
                            if (!jQuery(elCon).hasClass("grid-container")) return false;
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.forEach(function(element1) {
                            if (!element1.classList.contains("swiper_wrapper")) return true;
                            if (element1.querySelectorAll(".swiper-slide").length < 1) return true;
                            var elDirection = element1.getAttribute("data-direction") || "horizontal", elSpeed = element1.getAttribute("data-speed") || 300, elAutoPlay = element1.getAttribute("data-autoplay"), elAutoPlayDisableOnInteraction = element1.getAttribute("data-autoplay-disable-on-interaction") || true, elPauseOnHover = element1.getAttribute("data-hover"), elLoop = element1.getAttribute("data-loop"), elStart = element1.getAttribute("data-start") || 1, elEffect = element1.getAttribute("data-effect") || "slide", elGrabCursor = element1.getAttribute("data-grab"), elParallax = element1.getAttribute("data-parallax"), elAutoHeight = element1.getAttribute("data-autoheight"), slideNumberTotal = element1.querySelector(".slide-number-total"), slideNumberCurrent = element1.querySelector(".slide-number-current"), elVideoAutoPlay = element1.getAttribute("data-video-autoplay"), elSettings = element1.getAttribute("data-settings"), elPagination, elPaginationClickable;
                            ...
                            elStart = elStart == "random" ? Math.floor(Math.random() * element1.querySelectorAll(".swiper-slide:not(.swiper-slide-duplicate)").length) : Number(elStart) - 1;
                            if (element1.querySelector(".swiper-pagination")) {
                                elPagination = element1.querySelector(".swiper-pagination");
                                elPaginationClickable = true;
                            } else {
                                elPagination = "";
                                elPaginationClickable = false;
                            }
                            var elementNavNext = element1.querySelector(".slider-arrow-right"), elementNavPrev = element1.querySelector(".slider-arrow-left"), elementScollBar = element1.querySelector(".swiper-scrollbar");
                            var cnvsSwiper = new Swiper(element1.querySelector(".swiper-parent"), {
                                ...
                                on: {
                                    afterInit: function(swiper) {
                                        __base.sliderDimensions();
                                        if (element1.querySelectorAll(".yt-bg-player").length > 0) {
                                            element1.querySelectorAll(".yt-bg-player").forEach(function(el) {
                                        ...
                                        document.querySelectorAll(".swiper-slide-active [data-animate]").forEach(function(el) {
                                            var toAnimateDelay = el.getAttribute("data-delay"), toAnimateDelayTime = 0;
                                            if (toAnimateDelay) toAnimateDelayTime = Number(toAnimateDelay) + 750;
                                            else toAnimateDelayTime = 750;
                                        ...
                                        element1.querySelectorAll("[data-animate]").forEach(function(el) {
                                            var elementAnimation = el.getAttribute("data-animate");
                                            if (el.closest(".swiper-slide").classList.contains("swiper-slide-active")) return true;
                                            (elementAnimation + " animated").split(" ").forEach(function(_class) {
                                                el.classList.remove(_class);
                                            });
                                            el.classList.add("not-animated");
                                        });
                                        if (elAutoHeight) setTimeout(function() {
                                    ...
                                    transitionEnd: function(swiper) {
                                        if (slideNumberCurrent) {
                                            if (elLoop == true) slideNumberCurrent.innerHTML = Number(element1.querySelector(".swiper-slide.swiper-slide-active").getAttribute("data-swiper-slide-index")) + 1;
                                            else slideNumberCurrent.innerHTML = swiper.activeIndex + 1;
                                        }
                                        element1.querySelectorAll(".swiper-slide").forEach(function(slide) {
                                            if (slide.querySelector("video") && elVideoAutoPlay == true) slide.querySelector("video").pause();
                                            if (slide.querySelector(".yt-bg-player.mb_YTPlayer:not(.customjs)")) jQuery(slide).find(".yt-bg-player.mb_YTPlayer:not(.customjs)").YTPPause();
                                        });
                                        element1.querySelectorAll(".swiper-slide:not(.swiper-slide-active)").forEach(function(slide) {
                                            if (slide.querySelector("video")) {
                                                if (slide.querySelector("video").currentTime != 0) slide.querySelector("video").currentTime = 0;
                                            }
                                            var activeYTPlayer = slide.querySelector(".yt-bg-player.mb_YTPlayer:not(.customjs)");
                                            if (activeYTPlayer) jQuery(activeYTPlayer).YTPSeekTo(activeYTPlayer.getAttribute("data-start"));
                                        });
                                        if (element1.querySelector(".swiper-slide.swiper-slide-active").querySelector("video") && elVideoAutoPlay == true) element1.querySelector(".swiper-slide.swiper-slide-active").querySelector("video").play();
                                        if (element1.querySelector(".swiper-slide.swiper-slide-active").querySelector(".yt-bg-player.mb_YTPlayer:not(.customjs)") && elVideoAutoPlay == true) jQuery(element1).find(".swiper-slide.swiper-slide-active").find(".yt-bg-player.mb_YTPlayer:not(.customjs)").YTPPlay();
                                        element1.querySelectorAll(".swiper-slide.swiper-slide-active [data-animate]").forEach(function(el) {
                                            var toAnimateDelay = el.getAttribute("data-delay"), toAnimateDelayTime = 0;
                                            if (toAnimateDelay) toAnimateDelayTime = Number(toAnimateDelay) + 300;
                                            else toAnimateDelayTime = 300;
                                            if (!el.classList.contains("animated")) {
                                                el.classList.add("not-animated");
                                                var elementAnimation = el.getAttribute("data-animate");
                                                setTimeout(function() {
                            ...
                            if (slideNumberCurrent) {
                                if (elLoop == true) slideNumberCurrent.innerHTML = cnvsSwiper.realIndex + 1;
                                else slideNumberCurrent.innerHTML = cnvsSwiper.activeIndex + 1;
                            }
                            if (slideNumberTotal) slideNumberTotal.innerHTML = element1.querySelectorAll(".swiper-slide:not(.swiper-slide-duplicate)").length;
                        });
                    });
                }
            };
		 ...
		 */ SliderParallax: function() {
            ...
            var _parallax = function(_settings) {
                if (!_settings.sliderPx.el) return true;
                        ...
                        if (_settings.height + _settings.sliderPx.offset + 50 > _settings.scrollPos.y) {
                            ...
                            if (_settings.scrollPos.y > _settings.sliderPx.offset) {
                                if (typeof _settings.sliderPx.el.querySelector(".slider-inner") === "object") {
                                    _transform = (_settings.scrollPos.y - _settings.sliderPx.offset) * -0.4;
                                    _transform2 = (_settings.scrollPos.y - _settings.sliderPx.offset) * -0.15;
                            ...
                            } else if (_settings.sliderPx.el.querySelector(".slider-inner")) {
            ...
            var _offset = function() {
                ...
                var sliderParallaxOffsetTop = 0, headerHeight = __core.getVars.elHeader?.offsetHeight || 0;
                if (__core.getVars.elBody.classList.contains("side-header") || __core.getVars.elHeader && __core.getNext(__core.getVars.elHeader, ".include-header").length > 0) headerHeight = 0;
            ...
            var _setParallax = function(xPos, yPos, el) {
                if (el) fastdom.mutate(function() {
                    el.style.transform = "translate3d(" + xPos + ", " + yPos + "px, 0)";
                });
            };
            var _elementFade = function(_settings) {
                if (_settings.sliderPx.el.length < 1) return true;
                fastdom.mutate(function() {
                    if (_settings.body.classList.contains("is-expanded-menu") && !_settings.isMobile) {
                        var tHeaderOffset;
                        if (_settings.header && _settings.header.classList.contains("transparent-header") || _settings.body.classList.contains("side-header")) tHeaderOffset = 100;
                        else tHeaderOffset = 0;
                        if (_settings.sliderPx.el.classList.contains("slider-parallax-visible")) _settings.sliderPx.el.querySelectorAll(".slider-arrow-left,.slider-arrow-right,.slider-caption,.slider-element-fade").forEach(function(el) {
                            el.style.opacity = 1 - (_settings.scrollPos.y - tHeaderOffset) * 1.85 / _settings.height;
                        });
                    } else _settings.sliderPx.el.querySelectorAll(".slider-arrow-left,.slider-arrow-right,.slider-caption,.slider-element-fade").forEach(function(el) {
                        el.style.opacity = 1;
                    });
                });
            };
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elLazy = element1.find(".lazy"), elAnimation = element1.attr("data-animation") || "slide", elEasing = element1.attr("data-easing") || "swing", elDirection = element1.attr("data-direction") || "horizontal", elReverse = element1.attr("data-reverse"), elSlideshow = element1.attr("data-slideshow"), elPause = element1.attr("data-pause") || 5000, elSpeed = element1.attr("data-speed") || 600, elVideo = element1.attr("data-video"), elPagi = element1.attr("data-pagi"), elArrows = element1.attr("data-arrows"), elArrowLeft = element1.attr("data-arrow-left") || "uil uil-angle-left-b", elArrowRight = element1.attr("data-arrow-right") || "uil uil-angle-right-b", elThumbs = element1.attr("data-thumbs"), elHover = element1.attr("data-hover"), elSheight = element1.attr("data-smooth-height"), elTouch = element1.attr("data-touch"), elUseCSS = false;
                            if (elEasing == "swing") {
                                elEasing = "swing";
                                elUseCSS = true;
                            }
                            if (elReverse == "true") elReverse = true;
                            else elReverse = false;
                            if (elSlideshow == "false") elSlideshow = false;
                            else elSlideshow = true;
                            if (!elVideo) elVideo = false;
                            if (elSheight == "false") elSheight = false;
                            else elSheight = true;
                            if (elDirection == "vertical") elSheight = false;
                            if (elPagi == "false") elPagi = false;
                            else elPagi = true;
                            if (elThumbs == "true") elPagi = "thumbnails";
                            else elPagi;
                            if (elArrows == "false") elArrows = false;
                            else elArrows = true;
                            if (elHover == "false") elHover = false;
                            else elHover = true;
                            if (elTouch == "false") elTouch = false;
                            else elTouch = true;
                            element1.find(".flexslider").flexslider({
                                ...
                                start: function(slider) {
                                    ...
                                    setTimeout(function() {
                                        if (slider.parents(".grid-container.has-init-isotope").length > 0) slider.parents(".grid-container.has-init-isotope").isotope("layout");
                                    }, 1200);
                                    if (typeof skrollrInstance !== "undefined") skrollrInstance.refresh();
                                },
		 ...
		 */ FullVideo: function() {
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(element1) {
                        var elVideo = element1.querySelector("video"), elRatio = element1.getAttribute("data-ratio") || "16/9";
                        if (!elVideo) return true;
                        elRatio = elRatio.split("/");
                        elVideo.style.left = "";
                        elVideo.style.top = "";
                        var divWidth = element1.offsetWidth, divHeight = element1.offsetHeight, elWidth = Number(elRatio[0]) * divHeight / Number(elRatio[1]), elHeight = divHeight;
                        if (elWidth < divWidth) {
                            elWidth = divWidth;
                        ...
                        if (SEMICOLON.Mobile.any() && !element1.classList.contains("no-placeholder")) {
                            var placeholderImg = elVideo.getAttribute("poster") || elVideo.getAttribute("data-poster");
                            if (placeholderImg != "") element1.innerHTML += '<div class="video-placeholder" style="background-image: url(' + placeholderImg + ');"></div>';
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elVideo = element1.attr("data-video"), elMute = element1.attr("data-mute") || true, elRatio = element1.attr("data-ratio") || "16/9", elQuality = element1.attr("data-quality") || "hd720", elOpacity = element1.attr("data-opacity") || 1, elContainer = element1.attr("data-container") || "parent", elOptimize = element1.attr("data-optimize") || true, elLoop = element1.attr("data-loop") || true, elControls = element1.attr("data-controls") || false, elVolume = element1.attr("data-volume") || 50, elStart = element1.attr("data-start") || 0, elStop = element1.attr("data-stop") || 0, elAutoPlay = element1.attr("data-autoplay") || true, elFullScreen = element1.attr("data-fullscreen") || false, elCoverImage = element1.attr("data-coverimage") || "", elPauseOnBlur = element1.attr("data-pauseonblur") || true, elPlayIfVisible = element1.attr("data-playifvisible") || false;
                            if (elMute == "false") elMute = false;
                            if (elContainer == "parent") {
                                var parent1 = element1.parent();
                                if (parent1.attr("id")) elContainer = "#" + parent1.attr("id");
                                else {
                                    var ytPid = "yt-bg-player-parent-" + Math.floor(Math.random() * 10000);
                                    parent1.attr("id", ytPid);
                                    elContainer = "#" + ytPid;
                                }
                            }
                            if (elOptimize == "false") elOptimize = false;
                            if (elLoop == "false") elLoop = false;
                            if (elControls == "true") elControls = true;
                            if (elAutoPlay == "false") elAutoPlay = false;
                            if (elFullScreen == "true") elFullScreen = true;
                            if (elPauseOnBlur == "true") elPauseOnBlur = true;
                            if (elPlayIfVisible == "true") elPlayIfVisible = true;
                            element1.YTPlayer({
                                videoURL: elVideo,
                                mute: elMute,
                                ratio: elRatio,
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elSpeed = element1.attr("data-speed") || 300, elState = element1.attr("data-state");
                            if (elState != "open") element1.children(".toggle-content").hide();
                            else element1.addClass("toggle-active").children(".toggle-content").slideDown(Number(elSpeed));
                            element1.children(".toggle-header").off("click").on("click", function() {
                                element1.toggleClass("toggle-active").children(".toggle-content").slideToggle(Number(elSpeed));
                                return true;
                            });
                        });
                    });
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elState = element1.attr("data-state"), elActive = element1.attr("data-active") || 1, elActiveClass = element1.attr("data-active-class") || "", elCollapsible = element1.attr("data-collapsible") || "false", windowHash = location.hash, accActive;
                            elActive = Number(elActive) - 1;
                            if (typeof windowHash !== "undefined" && windowHash != "") {
                                accActive = element1.find(".accordion-header" + windowHash);
                                if (accActive.length > 0) elActive = accActive.index() / 2;
                            }
                            element1.find(".accordion-content").hide();
                            if (elState != "closed") element1.find(".accordion-header:eq(" + Number(elActive) + ")").addClass("accordion-active " + elActiveClass).next().show();
                            element1.find(".accordion-header").off("click").on("click", function() {
                                var clickTarget = jQuery(this);
                                if (clickTarget.next().is(":hidden")) {
                                    element1.find(".accordion-header").removeClass("accordion-active " + elActiveClass).next().slideUp("normal");
                                    clickTarget.toggleClass("accordion-active " + elActiveClass, true).next().stop(true, true).slideDown("normal", function() {
                                        if ((jQuery("body").hasClass("device-sm") || jQuery("body").hasClass("device-xs")) && element1.hasClass("scroll-on-open")) __core.scrollTo(__core.offset(clickTarget).top - __core.getVars.topScrollOffset - 40, 800, "easeOutQuad");
                                        __core.runContainerModules(clickTarget.next()[0]);
                                    });
                                } else if (elCollapsible == "true") clickTarget.toggleClass("accordion-active " + elActiveClass, false).next().stop(true, true).slideUp("normal");
                                return false;
                            });
                        });
                    });
		 ...
		 */ Counter: function() {
            var _run = function(elCounter, elFormat) {
                if (elFormat.comma == "true") {
                    var reFormat = "\\B(?=(\\d{" + elFormat.places + "})+(?!\\d))", regExp = new RegExp(reFormat, "g");
                ...
                } else elCounter.find("span").countTo();
            };
            return {
                init: function(selector) {
                    if (__core.getSelector(selector, false, false).length < 1) return true;
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elComma = element1.find("span").attr("data-comma"), elSep = element1.find("span").attr("data-sep") || ",", elPlaces = element1.find("span").attr("data-places") || 3;
                            var elCommaObj = {
                                comma: elComma,
                                sep: elSep,
                                places: Number(elPlaces)
                            };
                            if (element1.hasClass("counter-instant")) {
                                _run(element1, elCommaObj);
                                return;
                            }
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elFormat = element1.attr("data-format") || "dHMS", elSince = element1.attr("data-since"), elYear = element1.attr("data-year"), elMonth = element1.attr("data-month"), elDay = element1.attr("data-day"), elHour = element1.attr("data-hour"), elMin = element1.attr("data-minute"), elSec = element1.attr("data-second"), elRedirect = element1.attr("data-redirect"), dateFormat, setDate;
                            if (elYear) dateFormat = elYear;
                            if (elMonth && elMonth < 13) dateFormat = dateFormat + "-" + (elMonth < 10 ? "0" + elMonth : elMonth);
                            else if (elYear) dateFormat = dateFormat + "-01";
                            if (elDay && elDay < 32) dateFormat = dateFormat + "-" + (elDay < 10 ? "0" + elDay : elDay);
                            else if (elYear) dateFormat = dateFormat + "-01";
                            setDate = dateFormat != "" ? new Date(moment(dateFormat)) : new Date();
                            if (elHour && elHour < 25) setDate.setHours(setDate.getHours() + Number(elHour));
                            ...
                            if (!elRedirect) elRedirect = false;
                            if (elSince == "true") element1.countdown({
                                since: setDate,
                                format: elFormat,
                                expiryUrl: elRedirect
                            });
                            else element1.countdown({
                                until: setDate,
                                format: elFormat,
                                expiryUrl: elRedirect
                            });
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elLat = element1.attr("data-latitude"), elLon = element1.attr("data-longitude"), elAdd = element1.attr("data-address"), elCon = element1.attr("data-content"), elScroll = element1.attr("data-scrollwheel") || true, elType = element1.attr("data-maptype") || "ROADMAP", elZoom = element1.attr("data-zoom") || 12, elStyles = element1.attr("data-styles"), elMarkers = element1.attr("data-markers"), elIcon = element1.attr("data-icon"), elConPan = element1.attr("data-control-pan") || false, elConZoom = element1.attr("data-control-zoom") || false, elConMapT = element1.attr("data-control-maptype") || false, elConScale = element1.attr("data-control-scale") || false, elConStreetV = element1.attr("data-control-streetview") || false, elConOverview = element1.attr("data-control-overview") || false;
                            if (elAdd) elLat = elLon = false;
                            else if (!elLat && !elLon) {
                            ...
                            if (elMarkers) elMarkers = Function("return " + elMarkers)();
                            else if (elAdd) elMarkers = [
                                {
                                    address: elAdd,
                            ...
                            if (elIcon) elIcon = Function("return " + elIcon)();
                            else elIcon = {
		 ...
		 */ RoundedSkills: function() {
            var _run = function(element1, properties) {
                element1.easyPieChart({
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elSize = element1.attr("data-size") || 140, elSpeed = element1.attr("data-speed") || 2000, elWidth = element1.attr("data-width") || 4, elColor = element1.attr("data-color") || "#0093BF", elTrackColor = element1.attr("data-trackcolor") || "rgba(0,0,0,0.04)";
                            var properties = {
                                size: Number(elSize),
                                speed: Number(elSpeed),
                            ...
                            if (jQuery("body").hasClass("device-up-lg")) {
                                ...
                                var observer = new IntersectionObserver(function(entries, observer) {
                                    entries.forEach(function(entry) {
                                        if (entry.isIntersecting) {
                                            if (!element1.hasClass("skills-animated")) {
                                                setTimeout(function() {
                                                    element1.css({
                                                        opacity: 1
                                                    });
                                                }, 100);
                                                _run(element1, properties);
                                                element1.addClass("skills-animated");
                                            }
                                            observer.unobserve(entry.target);
                                        }
                                    });
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.forEach(function(element1) {
                            var elValue = element1.getAttribute("data-percent") || 90, elSpeed = element1.getAttribute("data-speed") || 1200, elBar = element1.querySelector(".skill-progress-percent");
                                ...
                                entries.forEach(function(entry) {
                                    if (entry.isIntersecting) {
                                        if (!elBar.classList.contains("skill-animated")) {
                                            __modules.counter(element1.querySelector(".counter"));
                                            if (element1.classList.contains("skill-progress-vertical")) {
		 ...
		 * --------------------------------------------------------------------------
		 */ Twitter: function() {
            var _build = function(tweet, element1, username) {
                var elFontClass = element1.getAttribute("data-font-class") || "font-body";
                var status = tweet.text.replace(/((https?|s?ftp|ssh)\:\/\/[^"\s\<\>]*[^.,;'">\:\s\<\>\)\]\!])/g, function(url) {
                    return '<a href="' + url + '" target="_blank">' + url + "</a>";
                }).replace(/\B@([_a-z0-9]+)/ig, function(reply) {
                    return reply.charAt(0) + '<a href="https://twitter.com/' + reply.substring(1) + '" target="_blank">' + reply.substring(1) + "</a>";
                });
                if (element1.classList.contains("fslider")) {
                    ...
                    element1.querySelector(".slider-wrap").append(slide);
                } else element1.innerHTML += '<li><i class="fa-brands fa-x-twitter"></i><div><span>' + status + '</span><small><a href="https://twitter.com/' + username + "/statuses/" + tweet.id_str + '" target="_blank">' + _time(tweet.created_at) + "</a></small></div></li>";
            };
            var _time = function(time_value) {
                ...
                else if (delta < 3600) return parseInt(delta / 60).toString() + " minutes ago";
                else if (delta < 7200) return "about an hour ago";
                else if (delta < 86400) return "about " + parseInt(delta / 3600).toString() + " hours ago";
                else if (delta < 172800) return "1 day ago";
            ...
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(element1) {
                        var elUser = element1.getAttribute("data-username") || "twitter", elCount = element1.getAttribute("data-count") || 3, elLoader = element1.getAttribute("data-loader") || "include/twitter/tweets.php", elFetch = element1.getAttribute("data-fetch-message") || "Fetching Tweets from Twitter...";
                        var alert = element1.querySelector(".twitter-widget-alert");
                        if (!alert) {
                            alert = document.createElement("div");
                            alert.classList.add("alert", "alert-warning", "twitter-widget-alert", "text-center");
                            element1.prepend(alert);
                        ...
                        fetch(elLoader + "?username=" + elUser).then(function(response) {
                            ...
                            if (typeof tweets === "object" && !tweets.isArray()) return false;
                            alert.remove();
                            var i = 0;
                            tweets?.some(function(tw) {
                                if (i == Number(elCount)) return;
                                _build(tw, element1, elUser);
                                i++;
                            });
                            if (element1.classList.contains("fslider")) var timer = setInterval(function() {
                                if (element1.querySelectorAll(".slide").length > 1) {
                                    element1.classList.remove("customjs");
                                    setTimeout(function() {
                                        __modules.flexSlider();
                                        jQuery(element1).find(".flexslider .slide").resize();
                                    }, 500);
                                    clearInterval(timer);
                                }
                            }, 1000);
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elID = element1.attr("data-id"), elCount = element1.attr("data-count") || 9, elType = element1.attr("data-type"), elTypeGet = "photos_public.gne";
                            if (elType == "group") elTypeGet = "groups_pool.gne";
                            element1.jflickrfeed({
                                feedapi: elTypeGet,
                                ...
                                element1.removeClass("customjs");
                                __core.imagesLoaded(element1[0]);
                                __modules.lightbox();
                                element1[0].addEventListener("CanvasImagesLoaded", function() {
                                    __modules.gridInit();
		 ...
		 * --------------------------------------------------------------------------
		 */ Instagram: function() {
            var _get = function(element1, loader, limit, fetchAlert) {
                var alert = element1.closest(".instagram-widget-alert");
                if (!alert) {
                    alert = document.createElement("div");
                    alert.classList.add("alert", "alert-warning", "instagram-widget-alert", "text-center");
                    element1.insertAdjacentElement("beforebegin", alert);
                ...
                fetch(loader).then(function(response) {
                    ...
                    if (images.length > 0) {
                        ...
                        for(var i = 0; i < limit; i++){
                            if (i === limit) continue;
                            var photo = images[i], thumb = photo.media_url;
                            if (photo.media_type === "VIDEO") thumb = photo.thumbnail_url;
                            element1.innerHTML += '<a class="grid-item" href="' + photo.permalink + '" target="_blank"><img src="' + thumb + '" alt="Image"></a>';
                        }
                    }
                    element1.classList.remove("customjs");
                    __core.imagesLoaded(element1);
                    element1.addEventListener("CanvasImagesLoaded", function() {
                        __modules.masonryThumbs();
                        __modules.lightbox();
                    });
                }).catch(function(err) {
            ...
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(element1) {
                        var elLimit = element1.getAttribute("data-count") || 12, elLoader = element1.getAttribute("data-loader") || "include/instagram/instagram.php", elFetch = element1.getAttribute("data-fetch-message") || "Fetching Photos from Instagram...";
                        if (Number(elLimit) > 12) elLimit = 12;
                        _get(element1, elLoader, elLimit, elFetch);
                    });
                }
            };
        }(),
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elSpeed = element1.attr("data-speed") || 250, elEasing = element1.attr("data-easing") || "swing", elArrow = element1.attr("data-arrow-class") || "fa-solid fa-angle-right";
                            element1.find("ul li:has(ul)").addClass("sub-menu");
                            element1.find("ul li:has(ul) > a").filter(":not(:has(.sub-menu-indicator))").append('<i class="sub-menu-indicator ' + elArrow + '"></i>');
                            if (element1.hasClass("on-hover")) element1.find("ul li:has(ul):not(.active)").hover(function(e) {
                            ...
                            else element1.find("ul li:has(ul) > a").off("click").on("click", function() {
                                var childElement = jQuery(this);
                                element1.find("ul li").not(childElement.parents()).removeClass("active");
                                ...
                                element1.find("ul li > ul").not(childElement.parent().children("ul")).not(childElement.parents("ul")).slideUp(Number(elSpeed), elEasing);
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elItems = element1.attr("data-items") || 4, elItemsXs = element1.attr("data-items-xs") || Number(elItems), elItemsSm = element1.attr("data-items-sm") || Number(elItemsXs), elItemsMd = element1.attr("data-items-md") || Number(elItemsSm), elItemsLg = element1.attr("data-items-lg") || Number(elItemsMd), elItemsXl = element1.attr("data-items-xl") || Number(elItemsLg), elItemsXxl = element1.attr("data-items-xxl") || Number(elItemsXl), elLoop = element1.attr("data-loop"), elAutoPlay = element1.attr("data-autoplay"), elSpeed = element1.attr("data-speed") || 250, elAnimateIn = element1.attr("data-animate-in"), elAnimateOut = element1.attr("data-animate-out"), elAutoWidth = element1.attr("data-auto-width"), elNav = element1.attr("data-nav"), elNavPrev = element1.attr("data-nav-prev") || '<i class="uil uil-angle-left-b"></i>', elNavNext = element1.attr("data-nav-next") || '<i class="uil uil-angle-right-b"></i>', elPagi = element1.attr("data-pagi"), elMargin = element1.attr("data-margin") || 20, elStage = element1.attr("data-stage-padding") || 0, elMerge = element1.attr("data-merge"), elStart = element1.attr("data-start") || 0, elRewind = element1.attr("data-rewind"), elSlideBy = element1.attr("data-slideby") || 1, elCenter = element1.attr("data-center"), elLazy = element1.attr("data-lazyload"), elVideo = element1.attr("data-video"), elRTL = element1.attr("data-rtl"), elAutoPlayTime = 5000, elAutoPlayHoverP = true;
                            if (elSlideBy == "page") elSlideBy = "page";
                            else elSlideBy = Number(elSlideBy);
                            if (elLoop == "true") elLoop = true;
                            else elLoop = false;
                            if (!elAutoPlay) {
                                elAutoPlay = false;
                            ...
                            if (elAutoWidth == "true") elAutoWidth = true;
                            else elAutoWidth = false;
                            if (elNav == "false") elNav = false;
                            else elNav = true;
                            if (elPagi == "false") elPagi = false;
                            else elPagi = true;
                            if (elRewind == "true") elRewind = true;
                            else elRewind = false;
                            if (elMerge == "true") elMerge = true;
                            else elMerge = false;
                            if (elCenter == "true") elCenter = true;
                            else elCenter = false;
                            if (elLazy == "true") elLazy = true;
                            else elLazy = false;
                            if (elVideo == "true") elVideo = true;
                            else elVideo = false;
                            if (elRTL == "true" || jQuery("body").hasClass("rtl")) elRTL = true;
                            else elRTL = false;
                            var carousel = element1.owlCarousel({
                                ...
                                onInitialized: function() {
                                    __base.sliderDimensions(element1.parents(".slider-element")[0]);
                                    __core.runContainerModules(element1[0]);
                                    if (element1.find(".owl-dot").length > 0) element1.addClass("with-carousel-dots");
                                }
                            });
                            jQuery(window).on("lazyLoadLoaded", function() {
                                if (element1.find(".lazy").length == element1.find(".lazy.lazy-loaded").length) {
                                    lazyLoadInstance.update();
                                    setTimeout(function() {
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elChildren = element1.children(), elBig = element1.attr("data-big");
                            if (elChildren.length < 1) return false;
                            ...
                            if (element1.filter(".has-init-isotope").length > 0) element1.isotope({
                                masonry: {
                            ...
                            setTimeout(function() {
                                element1.find(".grid-item-big").css({
                                    width: firstElementWidth * 2 + "px"
                                });
                            }, 500);
                            setTimeout(function() {
                                element1.filter(".has-init-isotope").isotope("layout");
                            }, 1000);
                            element1[0].addEventListener("transitionend", function() {
                                __modules.readmore();
                            });
                        });
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        selector = __core.getSelector(selector);
                        if (selector.length < 1) return true;
                        var element1 = selector, elPosition = element1.attr("data-notify-position") || "top-right", elType = element1.attr("data-notify-type"), elMsg = element1.attr("data-notify-msg") || "Please set a message!", elTimeout = element1.attr("data-notify-timeout") || 5000, elClose = element1.attr("data-notify-close") || "true", elAutoHide = element1.attr("data-notify-autohide") || "true", elId = "toast-" + Math.floor(Math.random() * 10000), elTrigger = element1.attr("data-notify-trigger") || "self", elTarget = element1.attr("data-notify-target"), elCloseHtml = "", elPosClass, elTypeClass, elCloseClass;
                        ...
                        if (elType == "info" || elType == "warning" || !elType) elCloseClass = "";
                        else elCloseClass = "btn-close-white";
                        ...
                        if (elAutoHide != "true") elAutoHide = false;
                        else elAutoHide = true;
                        let elTemplate = '<div class="position-fixed ' + elPosClass + ' p-3" style="z-index: 999999;">' + '<div id="' + elId + '" class="toast p-2 hide ' + elTypeClass + '" role="alert" aria-live="assertive" aria-atomic="true">' + '<div class="d-flex">' + '<div class="toast-body">' + elMsg + "</div>" + elCloseHtml + "</div>" + "</div>";
                        "</div>";
                        if (elTrigger == "self") {
                            if (!elTarget) {
                                element1.attr("data-notify-target", "#" + elId);
                        ...
                        var toastElement = element1.attr("data-notify-target"), toastInstance = jQuery(toastElement), elMsgOld = toastInstance.find(".toast-body");
                        if (jQuery(toastElement).length > 0) {
                            ...
                            toast.show();
                            if (elTrigger == "self") toastInstance.get(0).addEventListener("hidden.bs.toast", function() {
                                toastInstance.parent().remove();
                                element1.get(0).removeAttribute("data-notify-target");
                            });
                        }
                        return false;
                    });
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elTyped = element1.attr("data-typed") || "false", elRotator = element1.find(".t-rotate"), elAnimation = element1.attr("data-rotate") || "fade", elSpeed = element1.attr("data-speed") || 1200, elSep = element1.attr("data-separator") || ",";
                            if (elTyped == "true") {
                                var elTexts = elRotator.html().split(elSep), elLoop = element1.attr("data-loop") || "true", elShuffle = element1.attr("data-shuffle"), elCur = element1.attr("data-cursor") || "true", elSpeed = element1.attr("data-speed") || 50, elBackSpeed = element1.attr("data-backspeed") || 30, elBackDelay = element1.attr("data-backdelay");
                                if (elLoop == "true") elLoop = true;
                                else elLoop = false;
                                if (elShuffle == "true") elShuffle = true;
                                else elShuffle = false;
                                if (elCur == "true") elCur = true;
                                else elCur = false;
                            ...
                            } else var pluginData = elRotator.Morphext({
                                animation: elAnimation,
                                separator: elSep,
                                speed: Number(elSpeed)
                            });
		 ...
		 */ OnePage: function() {
            var _init = function(selector) {
                _hash();
                if (__core.getVars.elLinkScrolls) __core.getVars.elLinkScrolls.forEach(function(el) {
                ...
                if (__core.getVars.elOnePageMenus) __core.getVars.elOnePageMenus.forEach(function(onePageMenu) {
            ...
            var _hash = function() {
                if (__core.getOptions.scrollExternalLinks != true) return false;
                if (document.querySelector('a[data-href="' + __core.getVars.hash + '"]') || document.querySelector('a[data-scrollto="' + __core.getVars.hash + '"]')) {
                    ...
                    if (section) var int = setInterval(function() {
            ...
            var _getSection = function(el, type) {
                var anchor;
                if (type == "scrollTo") anchor = el.getAttribute("data-scrollto");
                else anchor = el.getAttribute("data-href");
            ...
            var _getSettings = function(el, type) {
                ...
                setTimeout(function() {
                    if (!section.hasAttribute("data-onepage-settings")) section.setAttribute("data-onepage-settings", JSON.stringify(settings));
            ...
            var _scroller = function(el, type, clicker = false) {
                var section = _getSection(el, type), sectionId = section.getAttribute("id"), settings;
                if (!section) return false;
                if (clicker == true) settings = _settings(section, el, false);
                else settings = JSON.parse(section.getAttribute("data-onepage-settings"));
            ...
            var _current = function() {
                ...
                if (typeof __core.getVars.pageSectionEls === "undefined") return true;
                __core.getVars.pageSectionEls.forEach(function(el) {
                    ...
                    if (settings) {
                        var h = __core.offset(el).top - settings.offset - 5, y = window.scrollY;
                        if (y >= h && y < h + el.offsetHeight && el.getAttribute("id") != currentOnePageSection && el.getAttribute("id")) currentOnePageSection = el.getAttribute("id");
                    }
                });
                return currentOnePageSection;
            };
            var _settings = function(section, element1, json = true) {
                var body = __core.getVars.elBody.classList;
                if (typeof section === "undefined" || element1.length < 1) return true;
                if (section.hasAttribute("data-onepage-settings") && json) return true;
                var defaults = {
                    offset: __core.getVars.topScrollOffset,
                    speed: 1250,
                    easing: false
                };
                var settings = {}, parentSettings = {}, parent1 = element1.closest(".one-page-menu");
                parentSettings.offset = parent1?.getAttribute("data-offset") || defaults.offset;
                parentSettings.speed = parent1?.getAttribute("data-speed") || defaults.speed;
                parentSettings.easing = parent1?.getAttribute("data-easing") || defaults.easing;
                var elementSettings = {
                    offset: element1.getAttribute("data-offset") || parentSettings.offset,
                    speed: element1.getAttribute("data-speed") || parentSettings.speed,
                    easing: element1.getAttribute("data-easing") || parentSettings.easing
                };
                var elOffsetXXL = element1.getAttribute("data-offset-xxl"), elOffsetXL = element1.getAttribute("data-offset-xl"), elOffsetLG = element1.getAttribute("data-offset-lg"), elOffsetMD = element1.getAttribute("data-offset-md"), elOffsetSM = element1.getAttribute("data-offset-sm"), elOffsetXS = element1.getAttribute("data-offset-xs");
            ...
            return {
                init: function(selector) {
                    ...
                    selector = __core.getSelector(selector, false);
                    if (selector.length < 1) return true;
                    var scrollToLinks = __core.filtered(selector, "[data-scrollto]"), onePageLinks = __core.filtered(selector, ".one-page-menu");
                    if (scrollToLinks.length > 0) __core.getVars.elLinkScrolls = scrollToLinks;
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), $body = jQuery("body"), elForm = element1.find("form"), elFormId = elForm.attr("id"), elAlert = element1.attr("data-alert-type"), elLoader = element1.attr("data-loader"), elResult = element1.find(".form-result"), elRedirect = element1.attr("data-redirect"), defaultBtn, alertType;
                            if (!elAlert) elAlert = "notify";
                            if (elFormId) $body.addClass(elFormId + "-ready");
                            element1.find("form").validate({
                                errorPlacement: function(error, elementItem) {
                                    if (elementItem.parents(".form-group").length > 0) error.appendTo(elementItem.parents(".form-group"));
                                    else error.insertAfter(elementItem);
                                },
                                focusCleanup: true,
                                submitHandler: function(form) {
                                    if (element1.hasClass("custom-submit")) {
                                        jQuery(form).submit();
                                        return true;
                                    }
                                    elResult.hide();
                                    ...
                                    } else jQuery(form).find(".form-process").fadeIn();
                                    ...
                                    jQuery(form).ajaxSubmit({
                                        ...
                                        success: function(data) {
                                            if (elLoader == "button") defaultBtn.html(defaultBtnText);
                                            else jQuery(form).find(".form-process").fadeOut();
                                            if (data.alert != "error" && elRedirect) {
                                                window.location.replace(elRedirect);
                                                return true;
                                            }
                                            if (elAlert == "inline") {
                                                if (data.alert == "error") alertType = "alert-danger";
                                            ...
                                            if (data.alert != "error") {
                                                ...
                                                if (typeof tinyMCE != "undefined" && tinyMCE.activeEditor && !tinyMCE.activeEditor.isHidden()) tinymce.activeEditor.setContent("");
                                                var rangeSlider = jQuery(form).find(".input-range-slider");
                                                if (rangeSlider.length > 0) rangeSlider.each(function() {
                                                    var range = jQuery(this).data("ionRangeSlider");
                                                    range.reset();
                                                });
                                                var ratings = jQuery(form).find(".input-rating");
                                                if (ratings.length > 0) ratings.each(function() {
                                                    jQuery(this).rating("reset");
                                                });
                                                var selectPicker = jQuery(form).find(".selectpicker");
                                                if (selectPicker.length > 0) selectPicker.each(function() {
                                            ...
                                            if (jQuery(form).find(".g-recaptcha").children("div").length > 0) grecaptcha.reset();
                                        }
                                    });
                                }
                            });
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        if (selector.length < 1) return true;
                        selector.each(function() {
                            var element1 = jQuery(this), elAlert = element1.attr("data-alert-type"), elLoader = element1.attr("data-loader"), elResult = element1.find(".widget-subscribe-form-result"), elRedirect = element1.attr("data-redirect"), defButton, defButtonText, alertType;
                            element1.find("form").validate({
                                submitHandler: function(form) {
                                    elResult.hide();
                                    if (elLoader == "button") {
                                    ...
                                    } else jQuery(form).find(".bi-envelope-plus").removeClass("bi-envelope-plus").addClass("bi-arrow-repeat icon-spin");
                                    jQuery(form).ajaxSubmit({
                                        target: elResult,
                                        dataType: "json",
                                        resetForm: true,
                                        success: function(data) {
                                            if (elLoader == "button") defButton.html(defButtonText);
                                            else jQuery(form).find(".bi-arrow-repeat").removeClass("bi-arrow-repeat icon-spin").addClass("bi-envelope-plus");
                                            if (data.alert != "error" && elRedirect) {
                                                window.location.replace(elRedirect);
                                                return true;
                                            }
		 ...
		 */ Conditional: function() {
            var _eval = function(field, value, conditions, check, target) {
                ...
                if (check == "validate") {
                    if (value) {
                        if (target.getAttribute("aria-invalid") == "false") fulfilled = true;
                        else fulfilled = false;
                    }
                } else switch(conditions.operator){
                    case "==":
                        if (value == conditions.value) fulfilled = true;
                        break;
                    case "!=":
                        if (value != conditions.value) fulfilled = true;
                        break;
                    ...
                    case ">=":
                        ...
                        break;
                    case "in":
                        if (conditions.value.includes(value)) fulfilled = true;
                        break;
                    default:
                        fulfilled = false;
                        break;
            ...
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(el) {
                        var field = el, condition = field.getAttribute("data-condition") || "==", conditionTarget = field.getAttribute("data-condition-target"), conditionValue = field.getAttribute("data-condition-value"), conditionCheck = field.getAttribute("data-condition-check") || "value", target = document.querySelector('[id*="' + conditionTarget + '"]'), value = target.value, targetType = target.type, eventType;
                        var conditions = {
                            operator: condition,
                        ...
                        if (targetType == "checkbox" || targetTag == "select" || targetType == "radio") eventType = "change";
                        ...
                        target.addEventListener(eventType, function() {
                            if (targetType == "checkbox") value = target.checked ? target.value : 0;
                            else if (targetType == "radio") value = target.checked ? target.value : "";
		 ...
		 */ ShapeDivider: function() {
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(element1) {
                        var elShape = element1.getAttribute("data-shape") || "valley", elWidth = element1.getAttribute("data-width") || 100, elHeight = element1.getAttribute("data-height") || 100, elFill = element1.getAttribute("data-fill"), elOut = element1.getAttribute("data-outside") || "false", elPos = element1.getAttribute("data-position") || "top", elId = "shape-divider-" + Math.floor(Math.random() * 10000), shape = "", width, height, fill, outside = "";
                        if (element1.classList.contains("shape-divider-complete")) return true;
                        if (elWidth < 100) elWidth = 100;
                        width = "width: calc( " + Number(elWidth) + "% + 1.5px );";
                        height = "height: " + Number(elHeight) + "px;";
                        fill = "fill: " + elFill + ";";
                        if (elOut == "true") {
                            if (elPos == "bottom") outside = "#" + elId + ".shape-divider { bottom: -" + (Number(elHeight) - 1) + "px; } ";
                            else outside = "#" + elId + ".shape-divider { top: -" + (Number(elHeight) - 1) + "px; } ";
                        }
                        var css = outside + "#" + elId + ".shape-divider svg { " + width + height + " } #" + elId + ".shape-divider .shape-divider-fill { " + fill + " }", style = document.createElement("style");
                        __core.getVars.elHead.appendChild(style);
                        style.appendChild(document.createTextNode(css));
                        element1.setAttribute("id", elId);
                        switch(elShape){
                            case "valley":
                        ...
                        element1.innerHTML = shape;
                        element1.querySelector("svg").classList.add("op-ts");
                        setTimeout(function() {
                            element1.querySelector("svg").classList.add("op-1");
                        }, 500);
                        element1.classList.add("shape-divider-complete");
                    });
                }
            };
        }(),
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        selector.each(function() {
                            var element1 = jQuery(this), elTop = element1.attr("data-offset-top") || 110, elBottom = element1.attr("data-offset-bottom") || 50;
                            element1.scwStickySidebar({
		 ...
		 */ Cookies: function() {
            ...
            return {
                init: function(selector) {
                    ...
                    if (cookieBar) {
                        if (cookieBar?.classList.contains("gdpr-settings-sm") && cookieBar?.classList.contains("gdpr-settings-right")) elDirection = "right";
                        else if (cookieBar?.classList.contains("gdpr-settings-sm")) elDirection = "left";
                        if (elDirection == "left") {
                            elSize = -elWidth;
                        ...
                        } else elSize = -elHeight;
                        cookieBar.style[elDirection] = elSize + "px";
                        if (__core.cookie.get("__cnvs_cookies_accept") != "1") setTimeout(function() {
                    ...
                    var gdprAcceptBtn = document.querySelectorAll(".gdpr-accept"), gdprDeclineBtn = document.querySelectorAll(".gdpr-decline"), gdprSaveCookies = document.querySelectorAll(".gdpr-save-cookies");
                    if (gdprAcceptBtn.length > 0) gdprAcceptBtn.forEach(function(btn) {
                    ...
                    if (gdprDeclineBtn.length > 0) gdprDeclineBtn.forEach(function(btn) {
                        btn.onclick = function(e) {
                            e.preventDefault();
                            if (cookieBar) {
                                cookieBar.style[elDirection] = elSize + "px";
                    ...
                    var acceptCookies = __core.cookie.get("__cnvs_cookies_accept"), declineCookies = __core.cookie.get("__cnvs_cookies_decline"), cookiesAllowed = (acceptCookies || acceptCookies != "0") && (!declineCookies || declineCookies != "1");
                    elSwitches?.forEach(function(el) {
                        var elCookie = "__cnvs_gdpr_" + el.getAttribute("data-cookie-name"), getCookie = __core.cookie.get(elCookie);
                        if (typeof getCookie !== "undefined" && getCookie == "1" && cookiesAllowed) el.checked = true;
                        else el.checked = false;
                    });
                    if (gdprSaveCookies.length > 0) gdprSaveCookies.forEach(function(btn) {
                        btn.onclick = function(e) {
                            e.preventDefault();
                            __core.cookie.set("__cnvs_cookies_accept", "1", elExpire);
                            __core.cookie.set("__cnvs_cookies_decline", "0", elExpire);
                            elSwitches.forEach(function(el) {
                                ...
                                if (el.checked == true) __core.cookie.set(elCookie, "1", elExpire);
                                else __core.cookie.remove(elCookie, "");
                            });
                            if (elSettings.closest(".mfp-content").length > 0) jQuery$.magnificPopup.close();
                            _resetPage(btn);
                        };
                    });
                    document.querySelectorAll(".gdpr-container")?.forEach(function(element1) {
                        var elCookie = "__cnvs_gdpr_" + element1.getAttribute("data-cookie-name"), elContent = element1.getAttribute("data-cookie-content"), elContentAjax = element1.getAttribute("data-cookie-content-ajax"), getCookie = __core.cookie.get(elCookie);
                        if (typeof getCookie !== "undefined" && getCookie == "1" && cookiesAllowed) {
                            element1.classList.add("gdpr-content-active");
                            if (elContentAjax) fetch(elContentAjax).then(function(response) {
                                return response.text();
                            }).then(function(html) {
                                var domParser = new DOMParser();
                                var parsedHTML = domParser.parseFromString(html, "text/html");
                                element1?.insertAdjacentHTML("beforeend", parsedHTML.body.innerHTML);
                            }).catch(function(err) {
                                console.log(err);
                            });
                            else if (elContent) element1.innerHTML += elContent;
                            __core.runContainerModules(element1);
                        } else element1.classList.add("gdpr-content-blocked");
                    });
                }
            };
        }(),
		 ...
		 */ Quantity: function() {
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(element1) {
                        var plus = element1.querySelector(".plus"), minus = element1.querySelector(".minus"), input = element1.querySelector(".qty");
                        var eventChange = new Event("change");
                        plus.onclick = function(e) {
                            e.preventDefault();
                            var value = input.value, step = input.getAttribute("step") || 1, max = input.getAttribute("max"), intRegex = /^\d+$/;
                            if (max && Number(value) >= Number(max)) return false;
                            if (intRegex.test(value)) {
                                var valuePlus = Number(value) + Number(step);
                                input.value = valuePlus;
                            } else input.value = Number(step);
                            input.dispatchEvent(eventChange);
                        };
                        minus.onclick = function(e) {
                            e.preventDefault();
                            var value = input.value, step = input.getAttribute("step") || 1, min = input.getAttribute("min"), intRegex = /^\d+$/;
                            if (!min || min < 0) min = 1;
                            if (intRegex.test(value)) {
		 ...
		 */ ReadMore: function() {
            ...
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(el) {
                        var element1 = el, elSize = element1.getAttribute("data-readmore-size") || "10rem", elSpeed = element1.getAttribute("data-readmore-speed") || 500, elScrollUp = element1.getAttribute("data-readmore-scrollup") || "false", elTrigger = element1.getAttribute("data-readmore-trigger") || ".read-more-trigger", elTriggerO = element1.getAttribute("data-readmore-trigger-open") || "Read More", elTriggerC = element1.getAttribute("data-readmore-trigger-close") || "Read Less", elMask;
                        element1.style.height = "";
                        element1.classList.remove("read-more-wrap-open");
                        var elHeight = element1.offsetHeight;
                        var elTriggerElement = element1.querySelector(elTrigger);
                        ...
                        element1.classList.add("read-more-wrap");
                        element1.style.height = elSize;
                        element1.style.transitionDuration = elSpeed + "ms";
                        if (!element1.querySelector(".read-more-mask")) element1.innerHTML += '<div class="read-more-mask"></div>';
                        elMask = element1.querySelector(".read-more-mask");
                        var elMaskD = element1.getAttribute("data-readmore-mask") || "true", elMaskColor = element1.getAttribute("data-readmore-maskcolor") || "#FFF", elMaskSize = element1.getAttribute("data-readmore-masksize") || "100%";
                        if (elMaskD == "true") {
                        ...
                        } else elMask.classList.add("d-none");
                        var elTriggerEl = element1.querySelector(elTrigger);
                        elTriggerEl.onclick = function(e) {
                            e.preventDefault();
                            if (element1.classList.contains("read-more-wrap-open")) {
                                element1.style.height = elSize;
                                element1.classList.remove("read-more-wrap-open");
                                elTriggerEl.innerHTML = elTriggerO;
                                setTimeout(function() {
                                    if (elScrollUp == "true") __core.scrollTo(element1.offsetTop - __core.getVars.topScrollOffset, false, false);
                                }, elSpeed);
                                if (elMaskD == "true") elMask.classList.add("op-ts", "op-1");
                            } else {
                                if (elTriggerC == "false") elTriggerEl.classList.add("d-none");
                                element1.style.height = elHeightN + "px";
                                element1.style.overflow = "";
                                element1.classList.add("read-more-wrap-open");
                                if (elTriggerEl) elTriggerEl.innerHTML = elTriggerC;
                                if (elMaskD == "true") {
                                    elMask.classList.remove("op-1");
                                    elMask.classList.add("op-0");
                                }
                            }
                            elTriggerEl = element1.querySelector(elTrigger);
                        };
                    });
		 ...
		 */ PricingSwitcher: function() {
            ...
            var _switcher = function(check, switcher, pricing, defClass, actClass) {
                var value;
                if (check.type == "checkbox") _staticValue = check.checked;
                else if (check.type == "radio") {
                    if (check.checked) _staticValue = check.value;
                } else _staticValue = check.value;
                value = _staticValue;
                ...
                if (check.type == "checkbox") value = value ? "true" : "false";
                defClass.split(" ").forEach(function(_class) {
            ...
            return {
                init: function(selector) {
                    ...
                    if (selector.length < 1) return true;
                    selector.forEach(function(element1) {
                        var elCheck = element1.querySelectorAll('[type="checkbox"], [type="radio"], select'), elDefClass = element1.getAttribute("data-default-class") || "text-muted op-05", elActClass = element1.getAttribute("data-active-class") || "fw-bold", elPricing = document.querySelector(element1.getAttribute("data-container"));
                        elCheck.forEach(function(el) {
                            _switcher(el, element1, elPricing, elDefClass, elActClass);
		 ...
		 */ AjaxTrigger: function() {
            var _load = function(params) {
                fetch(params.loader).then(function(response) {
                    ...
                    var domParser = new DOMParser();
                    var parsedHTML = domParser.parseFromString(html, "text/html");
                    if (params.placement == "append") params.container?.insertAdjacentHTML("beforeend", parsedHTML.body.innerHTML);
                    else params.container?.insertAdjacentHTML("afterbegin", parsedHTML.body.innerHTML);
                    if (params.triggerHide == "true") params.trigger.classList.add("d-none");
                    __core.runContainerModules(params.container);
                    __core.viewport();
                    if (params.triggerDisable == "true") setTimeout(function() {
            ...
            var _scripts = function(loadCSS = false, loadJS = false) {
                if (loadCSS) {
                    ...
                    if (loadCSS.length > 0) loadCSS.forEach(function(css) {
                        __core.loadCSS(css);
                    });
                }
                if (loadJS) {
                    loadJS = JSON.parse(loadJS);
                    if (loadJS.length > 0) loadJS.forEach(function(js) {
                        __core.loadJS(js);
                    });
                }
            };
            return {
                init: function(selector) {
                    ...
                    selector.forEach(function(el) {
                        ...
                        if (params.triggerType == "load") setTimeout(function() {
                            _load(params);
                        }, Number(params.loadDelay));
                        else params.trigger.onclick = function(e) {
                            e.preventDefault();
                            _load(params);
                        };
                    });
            ...
            return {
                init: function(selector) {
                    ...
                    selector.forEach(function(element1) {
                        element1.onclick = function(e) {
                            e.preventDefault();
                            var videoContent = element1.getAttribute("data-video-html"), videoRatio = element1.getAttribute("data-video-ratio") || "ratio ratio-16x9", videoPreviewEl = element1.querySelector(".video-facade-preview"), videoContentEl = element1.querySelector(".video-facade-content");
		 ...
		 */ SchemeToggle: function() {
            var _toggle = function(element1, sibling = false, action = false) {
                var bodyClassToggle = element1.getAttribute("data-bodyclass-toggle") || "dark";
                var classAdd = element1.getAttribute("data-add-class") || "scheme-toggler-active";
                var classRemove = element1.getAttribute("data-remove-class") || "scheme-toggler-active";
                var htmlAdd = element1.getAttribute("data-add-html");
                var htmlRemove = element1.getAttribute("data-remove-html");
                var toggleType = element1.getAttribute("data-type") || "trigger";
                var remember = element1.getAttribute("data-remember") || "false";
                if (__core.contains(bodyClassToggle, __core.getVars.elBody)) {
                    __core.classesFn("add", classAdd, element1);
                    __core.classesFn("remove", classRemove, element1);
                    element1.classList.add("body-state-toggled");
                    // Set Storage
                    if (remember == "true" && action) localStorage.setItem("cnvsBodyColorScheme", "dark");
                    if ("checkbox" == toggleType && sibling) element1.querySelector("input[type=checkbox]").checked = true;
                    else if (htmlAdd) element1.innerHTML = htmlAdd;
                } else {
                    __core.classesFn("add", classRemove, element1);
                    __core.classesFn("remove", classAdd, element1);
                    element1.classList.remove("body-state-toggled");
                    // Remove Storage
                    if (remember == "true" && action) localStorage.removeItem("cnvsBodyColorScheme");
                    if ("checkbox" == toggleType && sibling) element1.querySelector("input[type=checkbox]").checked = false;
                    else if (htmlRemove) element1.innerHTML = htmlRemove;
                }
                __base.setBSTheme();
                __modules.dataClasses();
            };
                ...
                init: function(selector) {
                    ...
                    selector.forEach(function(element1) {
                        ...
                        if ("checkbox" == toggleType) {
                            var elementCheck = element1.querySelector("input[type=checkbox]");
                            elementCheck.addEventListener("change", function() {
                                __core.classesFn("toggle", bodyClassToggle, __core.getVars.elBody);
                                _toggle(element1, false, true);
                                __core.siblings(element1, selector).forEach(function(el) {
                                    _toggle(el, true);
                                });
                            });
                        } else element1.onclick = function(e) {
                            e.preventDefault();
                            __core.classesFn("toggle", bodyClassToggle, __core.getVars.elBody);
                            _toggle(element1, false, true);
                            __core.siblings(element1, selector).forEach(function(el) {
            ...
            return {
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        selector = __core.getSelector(selector, false);
                        if (selector.length < 1) return true;
                        var clipboards = [], count = 0;
                        selector.forEach(function(el) {
                            var trigger = el.querySelector("button"), triggerText = trigger.innerHTML, copiedtext = trigger.getAttribute("data-copied") || "Copied", copiedTimeout = trigger.getAttribute("data-copied-timeout") || 5000;
            ...
            return {
                init: function(selector) {
                    __core.isFuncTrue(function() {
                        ...
                        if (!cond) return false;
                        if (typeof cnvsTips === "undefined" || cnvsTips.length < 1) return false;
                        __core.initFunction({
                            class: "has-plugin-tips",
                            event: "pluginTipsReady"
                        });
                        var body = __core.getVars.elBody.classList;
                        if (body.contains("init-plugin-tips")) return false;
                        ...
                        if (tipsDisable) tipsDisable.onclick = function(e) {
                            e.preventDefault();
                        ...
                        if (tipsEnable) tipsEnable.onclick = function(e) {
                            e.preventDefault();
                        ...
                        (function() {
                            var enabled = true;
                            if (__core.cookie.get("__cnvs_tips_cookies") == "hide") enabled = false;
                            if (enabled) {
		 ...
		 */ TextSplitter: function() {
            var _getText = function(element1) {
                return element1.textContent || element1.innerText;
            };
            ...
            var _words = function(element1, joiner = "span") {
                return _joiner(_getText(element1).split(" "), joiner, " ");
            };
            var _letters = function(element1, joiner = "span") {
                return _joiner(_getText(element1).split(""), joiner, "");
            };
		 ...
		 */ MediaActions: function() {
            ...
            var _volume = function(mediaEl) {
                ...
                if (mediaEl.volume < 0.1 || mediaEl.muted == true) mediaWrap.classList.add("media-is-muted");
                else mediaWrap.classList.remove("media-is-muted");
            };
                    ...
                    selector.forEach(function(mediaWrap) {
                        ...
                        _playEv.forEach(function(_event) {
                            mediaEl.addEventListener(_event, function() {
                                ...
                                _volume(mediaEl);
                                if (mediaDuration) mediaDuration.innerHTML = _time(mediaEl.currentTime);
                            });
                        });
                        ...
                        var ifLoaded = setInterval(function() {
                            if (mediaEl.readyState === 4) {
                                if (mediaDuration) mediaDuration.innerHTML = _time(mediaEl.duration);
                                clearInterval(ifLoaded);
                            }
                        }, 1000);
                        if (mediaTrigger) mediaTrigger.onclick = function(e) {
                            e.preventDefault();
                            if (mediaEl.paused) mediaEl.play();
                            else mediaEl.pause();
                        };
                        if (volumeTrigger) volumeTrigger.onclick = function(e) {
                            e.preventDefault();
                            if (mediaEl.muted) mediaEl.muted = false;
		 ...
		 */ ViewportDetect: function() {
            var _setBSTheme = function(target) {
                if (target.classList.contains("dark")) target.setAttribute("data-bs-theme", "dark");
                else target.removeAttribute("data-bs-theme");
            };
            return {
                init: function(selector) {
                    if (__core.getSelector(selector, false, false).length < 1) return true;
                    ...
                    selector.forEach(function(el) {
                        ...
                        var hasDark = false;
                        if (elClass.includes("dark") || elClassOut.includes("dark")) hasDark = true;
                        ...
                        var observer = new IntersectionObserver(function(el) {
                            el.forEach(function(entry) {
                                ...
                                if (!elClassTarget) elClassTarget = elTarget;
                ...
                var onScroll = function(params) {
                    fastdom.measure(function() {
                        ...
                        if (position >= params.start && position <= params.end) {
                            ...
                            ratio = 0;
                            start = end = 1;
                        } else percent = ratio = start = end = 0;
                        if (ratio > 0) params.elem.classList.add("scroll-detect-inview");
                        else params.elem.classList.remove("scroll-detect-inview");
                        if (start > 0 && start < 1) params.elem.classList.add("scroll-detect-inview-start");
                        else params.elem.classList.remove("scroll-detect-inview-start");
                        if (end > 0 && end < 1) params.elem.classList.add("scroll-detect-inview-end");
                        else params.elem.classList.remove("scroll-detect-inview-end");
                ...
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        var resizeObserver = new ResizeObserver(function(entries) {
                            entries.forEach(function(entry) {
                                if (!doing) var doing = setTimeout(function() {
                                    initParams(selector);
                                    doing = false;
                                }, 333);
                            });
                        ...
                        var initParams = function(selector) {
                            ...
                            selector.forEach(function(elem) {
                                var elemWidth = elem.offsetWidth, elemHeight = elem.offsetHeight, elemOffset = __core.offset(elem).top, viewportHeight = __core.getVars.viewport.height, includeWidth = elem.getAttribute("data-include-width"), includeHeight = elem.getAttribute("data-include-height"), includeOffset = elem.getAttribute("data-include-offset"), scrollOffset = elem.getAttribute("data-scroll-offset"), parallaxRatio = elem.getAttribute("data-parallax-ratio"), params = {};
                                if (scrollOffset) {
                                    scrollOffset = scrollOffset.split("%");
                                    if (scrollOffset.length > 1) elemOffset = elemOffset + viewportHeight * Number(scrollOffset[0]) * 0.01;
                                    else if (scrollOffset.length == 1 && scrollOffset[0]) elemOffset = elemOffset + Number(scrollOffset[0]);
                                }
                                var scrollStart = elemOffset - viewportHeight, scrollEnd = elemOffset + elemHeight, scrollRange = scrollEnd - scrollStart;
                                params.elem = elem;
                                ...
                                if (includeWidth == "true" || elem.classList.contains("parallax") && elem.getAttribute("data-parallax-direction") == "horizontal") elem.style.setProperty("--cnvs-scroll-width", params.width);
                                if (includeHeight == "true" || elem.classList.contains("parallax") && elem.getAttribute("data-parallax-direction") != "horizontal") elem.style.setProperty("--cnvs-scroll-height", params.height);
                                if (includeOffset == "true") elem.style.setProperty("--cnvs-scroll-offset", params.offset);
                                if (!isNaN(parallaxRatio)) elem.style.setProperty("--cnvs-parallax-ratio", parallaxRatio);
                                _percent(params);
            ...
            return {
                init: function(selector) {
                    ...
                    selector.forEach(function(elem) {
                        ...
                        if (defaultBtn) defaultBtn.onclick = function(e) {
                            e.preventDefault();
                            targetEl.style.fontSize = defaultSize + "px";
                        };
                        if (minusBtn) elem.querySelector(".font-size-minus").onclick = function(e) {
                            e.preventDefault();
                        ...
                        if (plusBtn) elem.querySelector(".font-size-plus").onclick = function(e) {
                            e.preventDefault();
                            var current = Number(document.defaultView.getComputedStyle(targetEl).getPropertyValue("font-size").split("px")[0]);
                            var newSize = current + percent;
                ...
                init: function(selector) {
                    ...
                    __core.isFuncTrue(function() {
                        ...
                        var tabTargetShow = function(target) {
                            ...
                            tabTrigger.show();
                            if (__core.getVars.hash && document.querySelector('[data-bs-target="' + __core.getVars.hash + '"]')) setTimeout(function() {
                        ...
                        if (__core.getVars.hash && document.querySelector('[data-bs-target="' + __core.getVars.hash + '"]')) tabTargetShow(document.querySelector('[data-bs-target="' + __core.getVars.hash + '"]'));
...
//# sourceMappingURL=index.fdf4e7ba.js.map
